...
void Optimizer::GlobalBundleAdjustemnt(Map* pMap, int nIterations, bool* pbStopFlag, bool bUseUniqueId, const unsigned long nLoopKF, const bool bRobust)
{
    vector<KeyFrame*> vpKFs = pMap->GetAllKeyFrames();
    vector<MapPoint*> vpMP = pMap->GetAllMapPoints();
    BundleAdjustment(vpKFs,vpMP,nIterations,pbStopFlag,bUseUniqueId,nLoopKF,bRobust);
}


void Optimizer::BundleAdjustment(const vector<KeyFrame *> &vpKFs, const vector<MapPoint *> &vpMP,
                                 int nIterations, bool* pbStopFlag, bool bUseUniqueId, const unsigned long nLoopKF, const bool bRobust)
{
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        ...
        cv::Mat p = pKF->GetPose();
        vSE3->setEstimate(Converter::toSE3Quat(pKF->GetPose()));
        if(bUseUniqueId){
            vSE3->setId(pKF->mnUniqueId);
            vSE3->setFixed(pKF->mnId==pMap->GetInitKFid() && pKF->mnClientId==pMap->mnClientId);
        }
        else{
            vSE3->setId(pKF->mnId);
        ...
        if(bUseUniqueId){
            if(pKF->mnUniqueId>maxKFid)
                maxKFid=pKF->mnUniqueId;
        }
        else{
            if(pKF->mnId>maxKFid)
                maxKFid=pKF->mnId;
    ...
    for(size_t i=0; i<vpMP.size(); i++)
    {
       ...
       const map<KeyFrame*,tuple<int,int>> observations = pMP->GetObservations();
        ...
        for(map<KeyFrame*,tuple<int,int>>::const_iterator mit=observations.begin(); mit!=observations.end(); mit++)
        {
            ...
            if(bUseUniqueId){
                if(pKF->isBad() || pKF->mnUniqueId>maxKFid)
                    continue;
                if(optimizer.vertex(id) == NULL || optimizer.vertex(pKF->mnUniqueId) == NULL)
                    continue;
            }
            else{
            ...
            if(leftIndex != -1 && pKF->mvuRight[get<0>(mit->second)]<0)
            {
                ...
                if(bUseUniqueId)
                    e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pKF->mnUniqueId)));
                else
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        ...
        if(bUseUniqueId)
            vSE3 = static_cast<g2o::VertexSE3Expmap*>(optimizer.vertex(pKF->mnUniqueId));
        else
            vSE3 = static_cast<g2o::VertexSE3Expmap*>(optimizer.vertex(pKF->mnId));

        g2o::SE3Quat SE3quat = vSE3->estimate();
        if(nLoopKF==pMap->GetOriginKF()->mnId) //TODO
        {
            pKF->SetPose(Converter::toCvMat(SE3quat), true);//What if the origin kf is not 0
        }
        else
        {
            /*if(!vSE3->fixed())
    ...
    for(size_t i=0; i<vpMP.size(); i++)
    {
        ...
        if(bUseUniqueId)
            vPoint = static_cast<g2o::VertexSBAPointXYZ*>(optimizer.vertex(pMP->mnUniqueId));
        else
...
void Optimizer::FullInertialBA(Map *pMap, int its, const bool bFixLocal, const long unsigned int nLoopId, bool bUseUniqueId, bool *pbStopFlag, bool bInit, float priorG, float priorA, Eigen::VectorXd *vSingVal, bool *bHess)
{
    long unsigned int maxKFid = pMap->GetMaxKFid();
    if(bUseUniqueId)
        maxKFid = pMap->GetMaxKFidUnique();
    std::cout<<"Optimizer::FullInertialBA maxKFid:"<<maxKFid<<std::endl;

    const vector<KeyFrame*> vpKFs = pMap->GetAllKeyFrames(true); //changed to sort
    std::cout<<"Optimizer::FullInertialBA vpKFs size:"<<vpKFs.size()<<std::endl;
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        ...
        //CHANGED
        if(pKFi->mnId == pMap->GetInitKFid() && pKFi->mnClientId == pMap->mnClientId)
            VP->setFixed(true);
        ...
        if(pKFi->bImu)
        {
            ...
            if(bUseUniqueId)
                VV->setId(maxKFid+3*(pKFi->mnUniqueId)+1);
            else
            ...
            if (!bInit)
            {
                ...
                if(bUseUniqueId)
                    VG->setId(maxKFid+3*(pKFi->mnUniqueId)+2);
                else
                ...
                if(bUseUniqueId)
                    VA->setId(maxKFid+3*(pKFi->mnUniqueId)+3);
                else
    ...
    for(size_t i=0;i<vpKFs.size();i++)
    {
        ...
        if(!pKFi->mPrevKF)
        {
            Verbose::PrintMess("FullInertialBA: NOT INERTIAL LINK TO PREVIOUS FRAME!", Verbose::VERBOSITY_DEBUG);
            //std::cout<<"pKFi:"<<pKFi->mnId<<std::endl;
            continue;
        }

        size_t pkfiId;
        if(bUseUniqueId)
            pkfiId = pKFi->mnUniqueId;
        else
            pkfiId = pKFi->mnId;
        if(pKFi->mPrevKF && pkfiId<=maxKFid)
        {
            size_t pkfiPrevId;
            if(bUseUniqueId)
                pkfiPrevId = pKFi->mPrevKF->mnUniqueId;
            else
                pkfiPrevId = pKFi->mPrevKF->mnId;
            if(pKFi->isBad() || pkfiPrevId>maxKFid)
                continue;
            if(pKFi->bImu && pKFi->mPrevKF->bImu)
            {
                pKFi->mpImuPreintegrated->SetNewBias(pKFi->mPrevKF->GetImuBias());
                g2o::HyperGraph::Vertex* VP1 = optimizer.vertex(pkfiPrevId);
                g2o::HyperGraph::Vertex* VV1 = optimizer.vertex(maxKFid+3*(pkfiPrevId)+1);
                ...
                if (!bInit)
                {
                    VG1 = optimizer.vertex(maxKFid+3*(pkfiPrevId)+2);
                    VA1 = optimizer.vertex(maxKFid+3*(pkfiPrevId)+3);
                    VG2 = optimizer.vertex(maxKFid+3*(pkfiId)+2);
                    VA2 = optimizer.vertex(maxKFid+3*(pkfiId)+3);
                }
                else
                {
                ...
                g2o::HyperGraph::Vertex* VP2 =  optimizer.vertex(pkfiId);
                g2o::HyperGraph::Vertex* VV2 = optimizer.vertex(maxKFid+3*(pkfiId)+1);
            ...
            {
                cout << pkfiId << " or " << pkfiPrevId << " no imu" << endl;
            }
        }
    }
    ...
    std::cout<<"FullInertialBA vpMPs.size():"<<vpMPs.size()<<std::endl;
    std::cout<<"FullInertialBA iniMPid:"<<iniMPid<<std::endl;
    ...
    for(size_t i=0; i<vpMPs.size(); i++)
    {
        ...
        unsigned long id;
        if(bUseUniqueId)
            id = pMP->mnUniqueId+iniMPid+1;
        else
            id = pMP->mnId+iniMPid+1;
        vPoint->setId(id);
        vPoint->setMarginalized(true);
        optimizer.addVertex(vPoint);
        ...
        for(map<KeyFrame*,tuple<int,int>>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(!pKFi->isBad())
            {
                ...
                if(leftIndex != -1 && pKFi->mvuRight[get<0>(mit->second)]<0) // Monocular observation
                {
                    ...
                    g2o::OptimizableGraph::Vertex* VP = dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pkfiId));

                    if(bAllFixed){
                        //std::cout<<"bAllFixed"<<std::endl;
                        if(!VP){
                    ...
                    optimizer.addEdge(e);
                //    std::cout<<"finish add edge"<<std::endl;
                }
    ...
    std::cout<<"FullInertialBA BEFORE Optimize"<<std::endl;
    optimizer.initializeOptimization();
    optimizer.optimize(its);

    std::cout<<"FullInertialBA FINISHED Optimize"<<std::endl;
    // Recover optimized data
    //Keyframes
    int count = 0;
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        ...
        if(pKFi->bImu) //CHANGED ITS POSITION
        {
            //std::cout<<"bImu"<<std::endl;
            VertexVelocity* VV = static_cast<VertexVelocity*>(optimizer.vertex(maxKFid+3*(pkfiId)+1));
            if(nLoopId==0)
            {
            ...
            if (!bInit)
            {
                VG = static_cast<VertexGyroBias*>(optimizer.vertex(maxKFid+3*(pkfiId)+2));
                VA = static_cast<VertexAccBias*>(optimizer.vertex(maxKFid+3*(pkfiId)+3));
            }
            else
            {
        ...
        VertexPose* VP = static_cast<VertexPose*>(optimizer.vertex(pkfiId));
        if(nLoopId==0)
        {
            //std::cout<<"setpose pkfi:"<<pKFi->mnId<<std::endl;
            ++count;
            cv::Mat Tcw = Converter::toCvSE3(VP->estimate().Rcw[0], VP->estimate().tcw[0]);
          //  std::cout<<"before pkfi"<<pKFi->mnId<<",pose"<<pKFi->GetPose()<<std::endl;
            //std::cout<<"before Tcw"<<pKFi->mnId<<",pose"<<Tcw<<std::endl;
            pKFi->SetPose(Tcw, true);
          //  std::cout<<"after pkfi"<<pKFi->mnId<<",pose"<<pKFi->GetPose()<<std::endl;
        }
        else
        {
    ...
    for(size_t i=0; i<vpMPs.size(); i++)
    {
        ...
        g2o::VertexSBAPointXYZ* vPoint;
        if(bUseUniqueId)
            vPoint = static_cast<g2o::VertexSBAPointXYZ*>(optimizer.vertex(pMP->mnUniqueId+iniMPid+1));
        else
...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, vector<KeyFrame*> &vpNonEnoughOptKFs)
{
    ...
    for(list<KeyFrame*>::iterator lit=lLocalKeyFrames.begin(), lend=lLocalKeyFrames.end(); lit!=lend; lit++)
    {
        ...
        vSE3->setId(Optimizer::GetID(pKFi->mnId,pKFi->mnClientId,true));
    ...
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        ...
        int id = Optimizer::GetID(pMP->mnId, pMP->mnClientId,false);
        vPoint->setId(id);
        vPoint->setMarginalized(true);
        optimizer.addVertex(vPoint);
        nPoints++;
        ...
        for(map<KeyFrame*,tuple<int,int>>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(!pKFi->isBad() && pKFi->GetMap() == pCurrentMap)
            {
                ...
                if(cam0Index != -1 && pKFi->mvuRight[cam0Index]<0)
                {
                    ...
                    e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(Optimizer::GetID(pKFi->mnId,pKFi->mnClientId,true))));
                    e->setMeasurement(obs);
                    const float &invSigma2 = pKFi->mvInvLevelSigma2[kpUn.octave];
                    e->setInformation(Eigen::Matrix2d::Identity()*invSigma2);
                ...
                else if(cam0Index != -1 && pKFi->mvuRight[cam0Index]>=0)// Stereo observation (with rectified images)
                {
                    ...
                    e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>
                        (optimizer.vertex(Optimizer::GetID(pKFi->mnId,pKFi->mnClientId,true))));
                    e->setMeasurement(obs);
                    const float &invSigma2 = pKFi->mvInvLevelSigma2[kpUn.octave];
                    Eigen::Matrix3d Info = Eigen::Matrix3d::Identity()*invSigma2;
                    e->setInformation(Info);
    ...
    for(list<KeyFrame*>::iterator lit=lLocalKeyFrames.begin(), lend=lLocalKeyFrames.end(); lit!=lend; lit++)
    {
        ...
        pKFi->SetPose(Converter::toCvMat(SE3quat), true);
...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, Map* pMap, int& num_fixedKF, int LocalBASize)
{
    // Local KeyFrames: First Breath Search from Current Keyframe
    list<KeyFrame*> lLocalKeyFrames;
    lLocalKeyFrames.push_back(pKF);
    ...
    Map* pCurrentMap = pKF->GetMap();
    const vector<KeyFrame*> vNeighKFs = pKF->GetVectorCovisibleKeyFrames();
    //const vector<KeyFrame*> vNeighKFs = pKF->GetBestCovisibilityKeyFrames(LocalBASize);
    for(int i=0, iend=vNeighKFs.size(); i<iend; i++)
    {
        KeyFrame* pKFi = vNeighKFs[i];
        pKFi->mnBALocalForKF = pKF->mnId;
    ...
    //Points
    //std::cout<<lLocalMapPoints.size()<<std::endl;
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        MapPoint* pMP = *lit;
        //g2o::VertexSBAPointXYZ* vPoint = static_cast<g2o::VertexSBAPointXYZ*>(optimizer.vertex(pMP->mnId+maxKFid+1));
        g2o::VertexSBAPointXYZ* vPoint = static_cast<g2o::VertexSBAPointXYZ*>
            (optimizer.vertex(Optimizer::GetID(pMP->mnId,pMP->mnClientId,false)));
        pMP->SetWorldPos(Converter::toCvMat(vPoint->estimate()), true);
        pMP->UpdateNormalAndDepth();
    }
                                       ...
                                       const map<KeyFrame *, set<KeyFrame *> > &LoopConnections, const bool &bFixScale)
{
    ...
    for(map<KeyFrame *, set<KeyFrame *> >::const_iterator mit = LoopConnections.begin(), mend=LoopConnections.end(); mit!=mend; mit++)
    {
        ...
        const long unsigned int nIDi = pKF->mnUniqueId;
        ...
        for(set<KeyFrame*>::const_iterator sit=spConnections.begin(), send=spConnections.end(); sit!=send; sit++)
        {
            ...
            const long unsigned int nIDj = (*sit)->mnUniqueId;
            //if((nIDi!=pCurKF->mnId || nIDj!=pLoopKF->mnId) && pKF->GetWeight(*sit)<minFeat)
            if((nIDi!=pCurKF->mnUniqueId || nIDj!=pLoopKF->mnUniqueId) && pKF->GetWeight(*sit)<minFeat)
                continue;
    ...
    for(size_t i=0, iend=vpKFs.size(); i<iend; i++)
    {
        ...
        for(set<KeyFrame*>::const_iterator sit=sLoopEdges.begin(), send=sLoopEdges.end(); sit!=send; sit++)
        {
            ...
            if(pLKF->mnUniqueId<pKF->mnUniqueId)//may be wrong
            {
                g2o::Sim3 Slw;
                ...
                if(itl!=NonCorrectedSim3.end())
                    ...
                    Slw = vScw[pLKF->mnUniqueId];
                    //Slw = vScw[pLKF->mnId];

                g2o::Sim3 Sli = Slw * Swi;
                g2o::EdgeSim3* el = new g2o::EdgeSim3();
                //el->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pLKF->mnId)));
                el->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pLKF->mnUniqueId)));
        ...
        for(vector<KeyFrame*>::const_iterator vit=vpConnectedKFs.begin(); vit!=vpConnectedKFs.end(); vit++)
        {
            ...
            if(pKFn && pKFn!=pParentKF && !pKF->hasChild(pKFn) && !sLoopEdges.count(pKFn))
            {
                ...
                if(!pKFn->isBad() && pKFn->mnUniqueId<pKF->mnUniqueId)
                {
                    //if(sInsertedEdges.count(make_pair(min(pKF->mnId,pKFn->mnId),max(pKF->mnId,pKFn->mnId))))
                    if(sInsertedEdges.count(make_pair(min(pKF->mnUniqueId,pKFn->mnUniqueId),max(pKF->mnUniqueId,pKFn->mnUniqueId))))
                        continue;
                    ...
                    en->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pKFn->mnUniqueId)));
        ...
        if(pKF->bImu && pKF->mPrevKF)
        {
            ...
            if(itp!=NonCorrectedSim3.end())
                ...
                Spw = vScw[pKF->mPrevKF->mnUniqueId];
            ...
            ep->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pKF->mPrevKF->mnUniqueId)));
    ...
    for(size_t i=0;i<vpKFs.size();i++)
    {
        ...
        const int nIDi = pKFi->mnUniqueId;
    ...
    for(size_t i=0, iend=vpMPs.size(); i<iend; i++)
    {
        ...
        if(pMP->mnCorrectedByKF==pCurKF->mnUniqueId)
        {
                                       ...
                                       vector<KeyFrame*> &vpNonFixedKFs, vector<MapPoint*> &vpNonCorrectedMPs)
{
    ...
    for(KeyFrame* pKFi : vpKFs)
    {
        ...
        {
            //int nIDj = pParentKFi->mnId;
            int nIDj = pParentKFi->mnUniqueId;
        ...
        for(set<KeyFrame*>::const_iterator sit=sLoopEdges.begin(), send=sLoopEdges.end(); sit!=send; sit++)
        {
            ...
            if(spKFs.find(pLKF) != spKFs.end() && pLKF->mnUniqueId<pKFi->mnUniqueId)
            {
                g2o::Sim3 Slw = vScw[pLKF->mnUniqueId];
                /*if(pLKF->mnMergeCorrectedForKF == pCurKF->mnId)
                {
                     Slw = vScw[pLKF->mnId];
                }
        ...
        for(vector<KeyFrame*>::const_iterator vit=vpConnectedKFs.begin(); vit!=vpConnectedKFs.end(); vit++)
        {
            ...
            if(pKFn && pKFn!=pParentKFi && !pKFi->hasChild(pKFn) && !sLoopEdges.count(pKFn) && spKFs.find(pKFn) != spKFs.end())
            {
                ...
                if(!pKFn->isBad() && pKFn->mnUniqueId<pKFi->mnUniqueId)
                {

                    g2o::Sim3 Snw =  vScw[pKFn->mnUniqueId];
                    /*if(pKFn->mnMergeCorrectedForKF == pCurKF->mnId)
                    {
                        Snw = vScw[pKFn->mnId];
                    }
...
void Optimizer::LocalInertialBA(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, bool bLarge, bool bRecInit)
{
    ...
    for(int i=0, iend=vpNeighsKFs.size(); i<iend; i++)
    {
        ...
        if(pKFi->mnClientId!=pKF->mnClientId)
    ...
    for(int i=0; i<N; i++)
    {
        ...
        VertexPose * VP = new VertexPose(pKFi);
        //VP->setId(pKFi->mnId);
        VP->setId(Optimizer::GetID(pKFi->mnId,pKFi->mnClientId,true));
        VP->setFixed(false);
        optimizer.addVertex(VP);
        ...
        if(pKFi->bImu)
        {
            VertexVelocity* VV = new VertexVelocity(pKFi);
            //VV->setId(maxKFid+3*(pKFi->mnId)+1);
            VV->setId(maxKFid + 3 * Optimizer::GetID(pKFi->mnId,pKFi->mnClientId,true)+1);
            VV->setFixed(false);
            optimizer.addVertex(VV);
            VertexGyroBias* VG = new VertexGyroBias(pKFi);
            //VG->setId(maxKFid+3*(pKFi->mnId)+2);
            VG->setId(maxKFid + 3 * Optimizer::GetID(pKFi->mnId,pKFi->mnClientId,true)+2);
            VG->setFixed(false);
            optimizer.addVertex(VG);
            VertexAccBias* VA = new VertexAccBias(pKFi);
            //VA->setId(maxKFid+3*(pKFi->mnId)+3);
            VA->setId(maxKFid + 3 * Optimizer::GetID(pKFi->mnId,pKFi->mnClientId,true)+3);
            VA->setFixed(false);
            optimizer.addVertex(VA);
        }
    }
    ...
    for(int i=0;i<N;i++)
    {
        ...
        if(!pKFi->mPrevKF)
        {
            cout << "LocalInertialBA: NOT INERTIAL LINK TO PREVIOUS FRAME!!!!" << endl;
            continue;
        }
        if(pKFi->bImu && pKFi->mPrevKF->bImu && pKFi->mpImuPreintegrated)
        {
            ...
            g2o::HyperGraph::Vertex* VP1 = optimizer.vertex(Optimizer::GetID(pKFi->mPrevKF->mnId,pKFi->mPrevKF->mnClientId,true));
            g2o::HyperGraph::Vertex* VV1 = optimizer.vertex(maxKFid+3*(Optimizer::GetID(pKFi->mPrevKF->mnId,pKFi->mPrevKF->mnClientId,true))+1);
            g2o::HyperGraph::Vertex* VG1 = optimizer.vertex(maxKFid+3*(Optimizer::GetID(pKFi->mPrevKF->mnId,pKFi->mPrevKF->mnClientId,true))+2);
            g2o::HyperGraph::Vertex* VA1 = optimizer.vertex(maxKFid+3*(Optimizer::GetID(pKFi->mPrevKF->mnId,pKFi->mPrevKF->mnClientId,true))+3);
            g2o::HyperGraph::Vertex* VP2 =  optimizer.vertex(Optimizer::GetID(pKFi->mnId,pKFi->mnClientId,true));
            g2o::HyperGraph::Vertex* VV2 = optimizer.vertex(maxKFid+3*(Optimizer::GetID(pKFi->mnId,pKFi->mnClientId,true))+1);
            g2o::HyperGraph::Vertex* VG2 = optimizer.vertex(maxKFid+3*(Optimizer::GetID(pKFi->mnId,pKFi->mnClientId,true))+2);
            g2o::HyperGraph::Vertex* VA2 = optimizer.vertex(maxKFid+3*(Optimizer::GetID(pKFi->mnId,pKFi->mnClientId,true))+3);
    ...
    for(int i=0;i<N;i++)
    {
        ...
        mVisEdges[Optimizer::GetID(pKFi->mnId,pKFi->mnClientId,true)] = 0;
    }
    for(list<KeyFrame*>::iterator lit=lFixedKeyFrames.begin(), lend=lFixedKeyFrames.end(); lit!=lend; lit++)
    {
        //mVisEdges[(*lit)->mnId] = 0;
        mVisEdges[Optimizer::GetID((*lit)->mnId,(*lit)->mnClientId,true)] = 0;
    }
    //std::cout<<"7"<<std::endl;
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        ...
        unsigned long id = Optimizer::GetID(pMP->mnId,pMP->mnClientId,false)+iniMPid+1;
        vPoint->setId(id);
        vPoint->setMarginalized(true);
        optimizer.addVertex(vPoint);
        const map<KeyFrame*,tuple<int,int>> observations = pMP->GetObservations();
        ...
        for(map<KeyFrame*,tuple<int,int>>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(pKFi->mnClientId!=pKF->mnClientId){
                //std::cout<<"obs has another."<<std::endl;
                bFromAnother=true;
                //continue;
            }
            ...
            if(!pKFi->isBad() && pKFi->GetMap() == pCurrentMap)
            {
                ...
                if(leftIndex != -1 && pKFi->mvuRight[leftIndex]<0)
                {
                    ...
                    mVisEdges[Optimizer::GetID(pKFi->mnId,pKFi->mnClientId,true)]++;
                    kpUn = pKFi->mvKeysUn[leftIndex];
                    Eigen::Matrix<double,2,1> obs;
                    obs << kpUn.pt.x, kpUn.pt.y;
                    /*if(bFromAnother)
                    ...
                    /*if(bFromAnother){
                        std::cout<<"unc2:"<<unc2<<std::endl;
                        std::cout<<"mvInvLevelSigma2:"<<pKFi->mvInvLevelSigma2[kpUn.octave]<<std::endl;
                    }*/
                    const float &invSigma2 = pKFi->mvInvLevelSigma2[kpUn.octave]/unc2;
                    e->setInformation(Eigen::Matrix2d::Identity()*invSigma2);
                    /*if(bFromAnother)
                        std::cout<<"setInformation:"<<Eigen::Matrix2d::Identity()*invSigma2<<std::endl;
                    */
                    g2o::RobustKernelHuber* rk = new g2o::RobustKernelHuber;
                    e->setRobustKernel(rk);
                    rk->setDelta(thHuberMono);
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(pKFi->bImu)
        {
            ...
            VertexVelocity* VV = static_cast<VertexVelocity*>
                        (optimizer.vertex(maxKFid+3*(Optimizer::GetID(pKFi->mnId,pKFi->mnClientId,true))+1));
            pKFi->SetVelocity(Converter::toCvMat(VV->estimate()));
            VertexGyroBias* VG = static_cast<VertexGyroBias*>
                        (optimizer.vertex(maxKFid+3*(Optimizer::GetID(pKFi->mnId,pKFi->mnClientId,true))+2));
            VertexAccBias* VA = static_cast<VertexAccBias*>
                        (optimizer.vertex(maxKFid+3*(Optimizer::GetID(pKFi->mnId,pKFi->mnClientId,true))+3));
        ...
        VertexPose* VP = static_cast<VertexPose*>
            (optimizer.vertex(Optimizer::GetID(pKFi->mnId,pKFi->mnClientId,true)));
        cv::Mat Tcw = Converter::toCvSE3(VP->estimate().Rcw[0], VP->estimate().tcw[0]);
        pKFi->SetPose(Tcw, false);
        pKFi->mnBALocalForKF=0;
    }
    //std::cout<<"before local visual"<<std::endl;
    // Local visual KeyFrame
    for(list<KeyFrame*>::iterator it=lpOptVisKFs.begin(), itEnd = lpOptVisKFs.end(); it!=itEnd; it++)
    {
        KeyFrame* pKFi = *it;
        //VertexPose* VP = static_cast<VertexPose*>(optimizer.vertex(pKFi->mnId));
        VertexPose* VP = static_cast<VertexPose*>(optimizer.vertex(Optimizer::GetID(pKFi->mnId,pKFi->mnClientId,true)));
        cv::Mat Tcw = Converter::toCvSE3(VP->estimate().Rcw[0], VP->estimate().tcw[0]);
        pKFi->SetPose(Tcw, false);
        pKFi->mnBALocalForKF=0;
    }
    //std::cout<<"after local visual"<<std::endl;
    //Points
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        MapPoint* pMP = *lit;
        //g2o::VertexSBAPointXYZ* vPoint = static_cast<g2o::VertexSBAPointXYZ*>(optimizer.vertex(pMP->mnId+iniMPid+1));
        g2o::VertexSBAPointXYZ* vPoint = static_cast<g2o::VertexSBAPointXYZ*>(optimizer.vertex(Optimizer::GetID(pMP->mnId,pMP->mnClientId,false)+iniMPid+1));
        pMP->SetWorldPos(Converter::toCvMat(vPoint->estimate()), false, false);
        pMP->UpdateNormalAndDepth();
    }
...
void Optimizer::MergeBundleAdjustmentVisual(KeyFrame* pCurrentKF, vector<KeyFrame*> vpWeldingKFs, vector<KeyFrame*> vpFixedKFs, bool *pbStopFlag)
{
    ...
    for(MapPoint* pMPi : vpMPs)
    {
        ...
        pMPi->SetWorldPos(Converter::toCvMat(vPoint->estimate()), true);
        pMPi->UpdateNormalAndDepth();
...
void Optimizer::LocalBundleAdjustment(KeyFrame* pMainKF,vector<KeyFrame*> vpAdjustKF, vector<KeyFrame*> vpFixedKF, bool *pbStopFlag)
{
    ...
    for(KeyFrame* pKFi : vpFixedKF)
    {
        ...
        pKFi->mnBALocalForMerge = pMainKF->mnUniqueId;

        g2o::VertexSE3Expmap * vSE3 = new g2o::VertexSE3Expmap();
        vSE3->setEstimate(Converter::toSE3Quat(pKFi->GetPose()));
        //vSE3->setId(pKFi->mnId);
        vSE3->setId(pKFi->mnUniqueId);
        vSE3->setFixed(true);
        optimizer.addVertex(vSE3);
        if(pKFi->mnId>maxKFid)
            maxKFid=pKFi->mnId;
        ...
        for(MapPoint* pMPi : spViewMPs)
        {
            if(pMPi)
                if(!pMPi->isBad() && pMPi->GetMap() == pCurrentMap)
                    ...
                    if(pMPi->mnBALocalForMerge!=pMainKF->mnUniqueId)
                    {
                        vpMPs.push_back(pMPi);
                        pMPi->mnBALocalForMerge=pMainKF->mnUniqueId;
                    }
    ...
    for(KeyFrame* pKFi : vpAdjustKF)
    {
        ...
        pKFi->mnBALocalForKF = pMainKF->mnUniqueId;
    ...
    for(unsigned int i=0; i < vpMPs.size(); ++i)
    {
        ...
        for(map<KeyFrame*,tuple<int,int>>::const_iterator mit=observations.begin(); mit!=observations.end(); mit++)
        {
            ...
            /*if(pKF->isBad() || pKF->mnId>maxKFid || pKF->mnBALocalForMerge != pMainKF->mnId || !pKF->GetMapPoint(get<0>(mit->second)))
                continue;*/
            if(pKF->isBad() || pKF->mnId>maxKFid || pKF->mnBALocalForMerge != pMainKF->mnUniqueId || !pKF->GetMapPoint(get<0>(mit->second)))
                continue;
    ...
    for(unsigned int i=0; i < vpMPs.size(); ++i)
    {
        ...
        for(map<KeyFrame*,tuple<int,int>>::const_iterator mit=observations.begin(); mit!=observations.end(); mit++)
        {
            ...
            /*if(pKF->isBad() || pKF->mnId>maxKFid || pKF->mnBALocalForKF != pMainKF->mnId || !pKF->GetMapPoint(get<0>(mit->second)))
                continue;*/
            if(pKF->isBad() || pKF->mnId>maxKFid || pKF->mnBALocalForKF != pMainKF->mnUniqueId || !pKF->GetMapPoint(get<0>(mit->second)))
                continue;
            const cv::KeyPoint &kpUn = pKF->mvKeysUn[get<0>(mit->second)];
            //cout << "-- KeyPoint loads" << endl;
    ...
    for(KeyFrame* pKFi : vpAdjustKF)
    {
        ...
        //g2o::VertexSE3Expmap* vSE3 = static_cast<g2o::VertexSE3Expmap*>(optimizer.vertex(pKFi->mnId));
        g2o::VertexSE3Expmap* vSE3 = static_cast<g2o::VertexSE3Expmap*>(optimizer.vertex(pKFi->mnUniqueId));
        g2o::SE3Quat SE3quat = vSE3->estimate();
        cv::Mat Tiw = Converter::toCvMat(SE3quat);
        cv::Mat Tco_cn = pKFi->GetPose() * Tiw.inv();
        cv::Vec3d trasl = Tco_cn.rowRange(0,3).col(3);
    ...
    for(MapPoint* pMPi : vpMPs)
    {
        ...
        /*g2o::VertexSBAPointXYZ* vPoint = static_cast<g2o::VertexSBAPointXYZ*>(optimizer.vertex(pMPi->mnId+maxKFid+1));*/
        g2o::VertexSBAPointXYZ* vPoint = static_cast<g2o::VertexSBAPointXYZ*>(optimizer.vertex(pMPi->mnUniqueId));
        pMPi->SetWorldPos(Converter::toCvMat(vPoint->estimate()), true);
        pMPi->UpdateNormalAndDepth();
...
void Optimizer::MergeInertialBA(KeyFrame* pCurrKF, KeyFrame* pMergeKF, bool *pbStopFlag, Map *pMap, LoopClosing::KeyFrameAndPose &corrPoses)
{
    ...
    const unsigned long maxKFid = pCurrKF->mnUniqueId;
    ...
    pCurrKF->mnBALocalForKF = pCurrKF->mnUniqueId;
    for(int i=1; i<Nd; i++)
    {
        if(vpOptimizableKFs.back()->mPrevKF)
        {
            ...
            vpOptimizableKFs.back()->mnBALocalForKF = pCurrKF->mnUniqueId;
        }
        else
            break;
    }
    ...
    if(vpOptimizableKFs.back()->mPrevKF)
    {
        ...
        vpOptimizableKFs.back()->mPrevKF->mnBALocalForKF=pCurrKF->mnUniqueId;
    }
    else
    {
    ...
    pMergeKF->mnBALocalForKF = pCurrKF->mnUniqueId;
    ...
    if(vpOptimizableKFs.back()->mPrevKF)
    {
        ...
        vpOptimizableKFs.back()->mPrevKF->mnBAFixedForKF=pCurrKF->mnUniqueId;
    }
    else
    {
        vpOptimizableKFs.back()->mnBALocalForKF=0;
        //vpOptimizableKFs.back()->mnBAFixedForKF=pCurrKF->mnId;
        vpOptimizableKFs.back()->mnBAFixedForKF=pCurrKF->mnUniqueId;
        lFixedKeyFrames.push_back(vpOptimizableKFs.back());
        vpOptimizableKFs.pop_back();
    }
        ...
        for(vector<MapPoint*>::iterator vit=vpMPs.begin(), vend=vpMPs.end(); vit!=vend; vit++)
        {
            ...
            if(pMP)
                if(!pMP->isBad()){
                    ...
                    if(pMP->mnBALocalForKF!=pCurrKF->mnUniqueId)
                    {
                        mLocalObs[pMP]=1;
                        lLocalMapPoints.push_back(pMP);
                        //pMP->mnBALocalForKF=pCurrKF->mnId;
                        pMP->mnBALocalForKF=pCurrKF->mnUniqueId;
                    }
                    else
                        mLocalObs[pMP]++;
                }
    ...
    for(vector<pair<MapPoint*,int>>::iterator lit=pairs.begin(), lend=pairs.end(); lit!=lend; lit++, i++)
    {
        ...
        for(map<KeyFrame*,tuple<int,int>>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(pKFi->mnBALocalForKF!=pCurrKF->mnUniqueId && pKFi->mnBAFixedForKF!=pCurrKF->mnUniqueId)
            {
                //pKFi->mnBALocalForKF=pCurrKF->mnId;
                pKFi->mnBALocalForKF=pCurrKF->mnUniqueId;
                if(!pKFi->isBad())
                {
    ...
    for(int i=0;i<N;i++)
    {
        ...
        if(!pKFi->mPrevKF)
        {
            Verbose::PrintMess("MergeInertialBA: NOT INERTIAL LINK TO PREVIOUS FRAME!!!!", Verbose::VERBOSITY_NORMAL);
            continue;
        }
        if(pKFi->bImu && pKFi->mPrevKF->bImu && pKFi->mpImuPreintegrated)
        {
            ...
            g2o::HyperGraph::Vertex* VG2 = optimizer.vertex(maxKFid+3*(pKFi->mnId)+2);
            g2o::HyperGraph::Vertex* VA2 = optimizer.vertex(maxKFid+3*(pKFi->mnId)+3);*/
            g2o::HyperGraph::Vertex* VP1 = optimizer.vertex(pKFi->mPrevKF->mnUniqueId);
            g2o::HyperGraph::Vertex* VV1 = optimizer.vertex(maxKFid+3*(pKFi->mPrevKF->mnUniqueId)+1);
            g2o::HyperGraph::Vertex* VG1 = optimizer.vertex(maxKFid+3*(pKFi->mPrevKF->mnUniqueId)+2);
            g2o::HyperGraph::Vertex* VA1 = optimizer.vertex(maxKFid+3*(pKFi->mPrevKF->mnUniqueId)+3);
            g2o::HyperGraph::Vertex* VP2 = optimizer.vertex(pKFi->mnUniqueId);
            g2o::HyperGraph::Vertex* VV2 = optimizer.vertex(maxKFid+3*(pKFi->mnUniqueId)+1);
            g2o::HyperGraph::Vertex* VG2 = optimizer.vertex(maxKFid+3*(pKFi->mnUniqueId)+2);
            g2o::HyperGraph::Vertex* VA2 = optimizer.vertex(maxKFid+3*(pKFi->mnUniqueId)+3);
    ...
    const unsigned long iniMPid = maxKFid*4; // TODO: should be  maxKFid*4;
    ...
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        ...
        //unsigned long id = pMP->mnId+iniMPid+1;
        unsigned long id = pMP->mnUniqueId+iniMPid+1;
        vPoint->setId(id);
        vPoint->setMarginalized(true);
        optimizer.addVertex(vPoint);
        ...
        for(map<KeyFrame*,tuple<int,int>>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(optimizer.vertex(id)==NULL || optimizer.vertex(pKFi->mnUniqueId)==NULL)
                continue;
            ...
            if(!pKFi->isBad())
            {
                ...
                if(pKFi->mvuRight[get<0>(mit->second)]<0) // Monocular observation
                {
                    ...
                    e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(id)));
                    //e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pKFi->mnId)));
                    e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pKFi->mnUniqueId)));
                    e->setMeasurement(obs);
                    const float &invSigma2 = pKFi->mvInvLevelSigma2[kpUn.octave];
                    e->setInformation(Eigen::Matrix2d::Identity()*invSigma2);
    ...
    for(int i=0; i<N; i++)
    {
        ...
        //VertexPose* VP = static_cast<VertexPose*>(optimizer.vertex(pKFi->mnId));
        VertexPose* VP = static_cast<VertexPose*>(optimizer.vertex(pKFi->mnUniqueId));
        cv::Mat Tcw = Converter::toCvSE3(VP->estimate().Rcw[0], VP->estimate().tcw[0]);
        pKFi->SetPose(Tcw, true);
        ...
        if(pKFi->bImu)
        {
            ...
            VertexGyroBias* VG = static_cast<VertexGyroBias*>(optimizer.vertex(maxKFid+3*(pKFi->mnId)+2));
            VertexAccBias* VA = static_cast<VertexAccBias*>(optimizer.vertex(maxKFid+3*(pKFi->mnId)+3));*/
            VertexVelocity* VV = static_cast<VertexVelocity*>(optimizer.vertex(maxKFid+3*(pKFi->mnUniqueId)+1));
            pKFi->SetVelocity(Converter::toCvMat(VV->estimate()));
            VertexGyroBias* VG = static_cast<VertexGyroBias*>(optimizer.vertex(maxKFid+3*(pKFi->mnUniqueId)+2));
            VertexAccBias* VA = static_cast<VertexAccBias*>(optimizer.vertex(maxKFid+3*(pKFi->mnUniqueId)+3));
            Vector6d b;
            b << VG->estimate(), VA->estimate();
            pKFi->SetNewBias(IMU::Bias(b[3],b[4],b[5],b[0],b[1],b[2]));
        }
    ...
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        MapPoint* pMP = *lit;
        //g2o::VertexSBAPointXYZ* vPoint = static_cast<g2o::VertexSBAPointXYZ*>(optimizer.vertex(pMP->mnId+iniMPid+1));
        g2o::VertexSBAPointXYZ* vPoint = static_cast<g2o::VertexSBAPointXYZ*>(optimizer.vertex(pMP->mnUniqueId+iniMPid+1));
        pMP->SetWorldPos(Converter::toCvMat(vPoint->estimate()), true);
        pMP->UpdateNormalAndDepth();
    }
...
int Optimizer::PoseInertialOptimizationLastFrame(Frame *pFrame, bool bRecInit)
{
    ...
    // Set MapPoint vertices
    const int N = pFrame->N;
    //std::cout<<"Optimizer N:"<<N<<std::endl;
    const int Nleft = pFrame->Nleft;
    const bool bRight = (Nleft!=-1);
                                       ...
                                       const map<KeyFrame *, set<KeyFrame *> > &LoopConnections)
{
    ...
    for(size_t i=0, iend=vpKFs.size(); i<iend; i++)
    {
        ...
        for(set<KeyFrame*>::const_iterator sit=sLoopEdges.begin(), send=sLoopEdges.end(); sit!=send; sit++)
        {
            KeyFrame* pLKF = *sit;
            //if(pLKF->mnId<pKF->mnId)
            if(pLKF->mnUniqueId<pKF->mnUniqueId)
            {
                g2o::Sim3 Swl;
                ...
                if(itl!=NonCorrectedSim3.end())
                    ...
                    Swl = vScw[pLKF->mnUniqueId].inverse();
                ...
                e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pLKF->mnUniqueId)));
                e->information() = matLambda;
                optimizer.addEdge(e);
            }
        }
            ...
            if(pKFn && pKFn!=pParentKF && pKFn!=prevKF && pKFn!=pKF->mNextKF && !pKF->hasChild(pKFn) && !sLoopEdges.count(pKFn))
            {
                ...
                if(!pKFn->isBad() && pKFn->mnUniqueId<pKF->mnUniqueId)
                {
                    ...
                    if(itn!=NonCorrectedSim3.end())
                        ...
                        Swn = vScw[pKFn->mnUniqueId].inverse();
                    ...
                    e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(nIDi)));
                    e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pKFn->mnUniqueId)));
