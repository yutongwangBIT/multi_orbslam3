...
LoopClosing::LoopClosing(CentralControl* pCC, Atlas *pAtlas, KeyFrameDatabase *pDB, ORBVocabulary *pVoc, eSensor sensor):
    mbResetRequested(false), mbResetActiveMapRequested(false), mbFinishRequested(false), mbFinished(true), mpAtlas(pAtlas),
    mpKeyFrameDB(pDB), mpORBVocabulary(pVoc), mpMatchedKF(NULL), mLastLoopKFid(0), mbRunningGBA(false), mbFinishedGBA(true),
    mbStopGBA(false), mpThreadGBA(NULL), /*mbFixScale(bFixScale), */mnFullBAIdx(0), mnLoopNumCoincidences(0), mnMergeNumCoincidences(0),
    mbLoopDetected(false), mbMergeDetected(false), mnLoopNumNotFound(0), mnMergeNumNotFound(0), mpCC(pCC), mSensor(sensor)
{
...
void LoopClosing::Run()
{
    ...
    while(1)
    {
        //while(!mpCC->LockPlaceRec()){usleep(1000);}
        ...
        if(CheckNewKeyFrames())
        {
            ...
            if(NewDetectCommonRegions())
            {
                if(mbMergeDetected)
                {
                    ...
                    if ((mSensor==IMU_MONOCULAR ||mSensor==IMU_STEREO) &&
                            (!mpCurrentKF->GetInertialBA2() || !mpMergeMatchedKF->GetInertialBA2()))
                    {
                        cout << "IMU is not initilized, merge is aborted" << endl;
                    }
                    else
                    {
                        cout<<GREEN<<"*Merged detected"<<RST<<endl;
                        ...
                        {
                            std::cout<<GREEN<<mSold_new.scale()<<RST<<std::endl;
                            ...
                            if ((mSensor==IMU_MONOCULAR ||mSensor==IMU_STEREO) && mpCurrentKF->GetInertialBA2())
                                  // mpCurrentKF->GetMap()->GetInertialBA1()) // TODO, maybe with GetInertialBA1 //Tongjiang:2
                            {
...
//                        cout << "Angle Rw2w1: " << 180*LogSO3(mSold_new.rotation().toRotationMatrix())/3.14 << endl;
                        cout <<GREEN<< "scale w2w1: " << mSold_new.scale() << RST <<endl; //Why =1???
                        ...
                        if (mSensor==IMU_MONOCULAR ||mSensor==IMU_STEREO)
                            ...
                            //while(!mpAtlas->LockViewer()){usleep(1000);}
                            MergeLocal();
                            //mpAtlas->UnLockViewer();
                        }
                    }
                ...
                if(mbLoopDetected)
                {
                    ...
                    cout<<GREEN<<"*Loop detected"<<RST<<endl;
                    ...
                    if(mpCurrentKF->GetMap()->IsInertial())
                    {
                        ...
                        if (fabs(phi(0))<0.008f && fabs(phi(1))<0.008f && fabs(phi(2))<0.349f)
                        {
                            if(mpCurrentKF->GetMap()->IsInertial())
                            {
                                // If inertial, force only yaw
                                if ((mSensor==IMU_MONOCULAR ||mSensor==IMU_STEREO) && mpCurrentKF->GetInertialBA2())
                                        //mpCurrentKF->GetMap()->GetInertialBA2()) // TODO, maybe with GetInertialBA1
                                {
                                    phi(0)=0;
                                    phi(1)=0;
                    ...
                    else
                    {
                        ...
                        CorrectLoop();
                        std::cout<<"LC: corrected Loop"<<std::endl;
                    }
...
bool LoopClosing::CheckNewKeyFrames()
{
    ...
    /*if(!mlpLoopKeyFrameQueue.empty())
        std::cout<<"LC:KF size:"<<mlpLoopKeyFrameQueue.size()<<", Client:"<<unsigned(mpAtlas->mnClientId)<<std::endl;*/
...
bool LoopClosing::NewDetectCommonRegions()
{
    ...
    //if(mpLastMap->IsInertial() && !mpLastMap->GetInertialBA1())
    if(mpLastMap->IsInertial() && !mpCurrentKF->GetInertialBA1())
    {
    ...
    //CHANGED
    if(mpLastMap->IsInertial() && !mpCurrentKF->GetInertialBA2())
    {
        mpKeyFrameDB->add(mpCurrentKF);
        mpCurrentKF->SetErase();
        return false;
    }
    if(mSensor == STEREO && mpLastMap->GetAllKeyFrames().size() < 5) //12
    {
        mpKeyFrameDB->add(mpCurrentKF);
        mpCurrentKF->SetErase();
        return false;
    ...
    {
        mpKeyFrameDB->add(mpCurrentKF);
        std::cout<<"Erase kf cause <12"<<std::endl;
        mpCurrentKF->SetErase();
        return false;
    }
    ...
    if(mnLoopNumCoincidences > 0)
    {
        ...
        std::cout<<"loop before DetectAndReffineSim3FromLastKF"<<std::endl;
        ...
        if(bCommonRegion)
        {
            std::cout<<"loop bCommonRegion"<<std::endl;
    ...
    if(mnMergeNumCoincidences > 0)
    {
        ...
        if(bCommonRegion)
        {
            cout << "BoW: Merge reffined Sim3 transformation succesfull" << endl;
    ...
    if(mbMergeDetected || mbLoopDetected)
    {
        ...
        mpKeyFrameDB->add(mpCurrentKF);
        std::cout<<"in detect Loop Detected"<<std::endl;
        return true;
    }
    //-------------
    //TODO: This is only necessary if we use a minimun score for pick the best candidates
    const vector<KeyFrame*> vpConnectedKeyFrames = mpCurrentKF->GetVectorCovisibleKeyFrames();
    if(vpConnectedKeyFrames.empty()){
        mpCurrentKF->SetErase();
        std::cout<<"LC: KF erased because of no connected kfs"<<std::endl;
        return false;
    }
...
/*#ifdef COMPILEDWITHC11
    ...
    {
        //std::cout<<"vpLoopBowCand:"<<vpLoopBowCand.size()<<std::endl;
        mbLoopDetected = DetectCommonRegionsFromBoW(vpLoopBowCand, mpLoopMatchedKF, mpLoopLastCurrentKF, mg2oLoopSlw, mnLoopNumCoincidences, mvpLoopMPs, mvpLoopMatchedMPs);
        if(mbLoopDetected)
            cout << "LC: mbLoopDetected" << endl;
    }
    // Merge candidates
    ...
    if(!bMergeDetectedInKF && !vpMergeBowCand.empty())
    {
        ...
        if(mbMergeDetected)
            cout << "LC: DetectCommonRegionsFromBoW" << endl;
    }
                                                 ...
                                                 std::vector<MapPoint*> &vpMPs, std::vector<MapPoint*> &vpMatchedMPs)
{
    ...
    if(nNumProjMatches >= nProjMatches)
    {
        ...
        //if(mSensor==IMU_MONOCULAR && !pCurrentKF->GetMap()->GetInertialBA2())
        if(mSensor==IMU_MONOCULAR && !pCurrentKF->GetInertialBA2())
            bFixedScale=false;
        cout << "REFFINE-SIM3: before OptimizeSim3"<< endl;
        int numOptMatches = Optimizer::OptimizeSim3(mpCurrentKF, pMatchedKF, vpMatchedMPs, gScm, 10, bFixedScale, mHessian7x7, true);
        cout << "REFFINE-SIM3: Optimize Sim3 from last KF with " << numOptMatches << " inliers" << endl;
//TONGJIANG: gScm not used?
                                             ...
                                             int &nNumCoincidences, std::vector<MapPoint*> &vpMPs, std::vector<MapPoint*> &vpMatchedMPs)
{
    ...
    if(spConnectedKeyFrames.empty()){
        std::cout<<"LC:DetectCommonRegionsFromBoW KF is erased because of no connected kfs"<<std::endl;
        mpCurrentKF->SetErase();
        return false;
    }
    ...
    int index = 0;
    //std::cout<<"before loop, candidates size:"<<vpBowCand.size()<<std::endl;
    for(KeyFrame* pKFi : vpBowCand)
    {
        ...
        if(vpCovKFi.empty()){
           std::cout<<"LC:vpCovKFi is empty"<<std::endl;
           pKFi->SetErase();
           return false;
        }
        else{
           //std::cout<<"LC:vpCovKFi size:"<<vpCovKFi.size()<<std::endl;
        }
        ...
        for(int j=0; j<vpCovKFi.size(); ++j)
        {
            ...
            if (num > nMostBoWNumMatches)
            {
                ...
                nIndexMostBoWMatchesKF = j; //TONGJIANG:nMostBoWNumMatches und nIndexMostBoWMatchesKF werden nicht weiter benutzt.
            }
        }
        ...
        /*    cout << "Geometric validation with " << numBoWMatches << endl;
            ...
            cout << "vpMatchedPoints:"<<vpMatchedPoints.size()<<std::endl;*/
            // Geometric validation

            bool bFixedScale = mbFixScale;
            //if(mSensor==IMU_MONOCULAR && !mpCurrentKF->GetMap()->GetInertialBA2())
            if(mSensor==IMU_MONOCULAR && !mpCurrentKF->GetInertialBA2())
                bFixedScale=false;
            ...
            if(bConverge)
            {
              //  cout <<"BoW: " << nInliers << " inliers in Sim3Solver" << endl;
                ...
                if(vpCovKFi.empty()){
                    std::cout<<"LC: pMostBoWMatchesKF hsa no BestCovisible KFs"<<std::endl;
                    pMostBoWMatchesKF->SetErase();
                    return false;
                }
                ...
                if(numProjMatches >= nProjMatches)
                {
                    ...
                    if(numOptMatches >= nSim3Inliers)
                    {
                        ...
                        if(numProjOptMatches >= nProjOptMatches)
                        {
                            ...
                            //std::cout<<"LC:vpCurrentCovKFs size:"<<vpCurrentCovKFs.size()<<std::endl;
                            //cout << "---" << endl;
    ...
    //std::cout<<"end loop with index:"<<index<<std::endl;
...
void LoopClosing::CorrectLoop()
{
    ...
    std::set<CentralControl*> spCC = mpCurrentKF->GetMap()->GetCCPtrs();
    for(std::set<CentralControl*>::const_iterator sit = spCC.begin();sit!=spCC.end();++sit)
    {
        CentralControl* pCC = *sit;
        std::cout<<"LOCKCOMM FOR LOOP CLOSE pcc_current client id:"<<unsigned(pCC->mnClientId)<<std::endl;
        while(!pCC->LockComm()){usleep(1000);}
      //  while(!pCC->LockMapping()){usleep(1000);}
    }
...

    {
        ...
        for(KeyFrameAndPose::iterator mit=CorrectedSim3.begin(), mend=CorrectedSim3.end(); mit!=mend; mit++)
        {
            ...
            for(size_t iMP=0, endMPi = vpMPsi.size(); iMP<endMPi; iMP++)
            {
                ...
                if(pMPi->mnCorrectedByKF==mpCurrentKF->mnUniqueId)
                    continue;
                ...
                pMPi->mnCorrectedByKF = mpCurrentKF->mnUniqueId;
                pMPi->mnCorrectedReference = pKFi->mnUniqueId;
    ...
    if(!pLoopMap->isImuInitialized() || (pLoopMap->KeyFramesInMap()<200 && mpAtlas->CountMaps()<4))
    {
        mbRunningGBA = true;
        mbFinishedGBA = false;
        mbStopGBA = false;
    ...
    //CHANGED FOR COMMUNICATION
    std::cout << "LC: Wait for GBA to finish" << std::endl;
    while(isRunningGBA()) {
        usleep(10000);
    }
    std::cout << "LC: GBA finished - continue" << std::endl;
    //mpAtlas->UnLockMapUpdate();
    //mpCC->UnLockComm();
    for(std::set<CentralControl*>::const_iterator sit = spCC.begin();sit!=spCC.end();++sit)
    {
        CentralControl* pCC = *sit;
        std::cout<<"UnLOCKCOMM FOR LOOP CLOSE pcc_current client id:"<<unsigned(pCC->mnClientId)<<std::endl;
        pCC->UnLockComm();
        //pCC->UnLockMapping();
    }
...
void LoopClosing::MergeLocal()
{
    ...
    //bool bMergeCCs=false;
    std::set<CentralControl*> spCC_merge;
    std::set<CentralControl*> spCC_current;
    ...
    Verbose::PrintMess("MERGE-VISUAL: Request Stop Local Mapping", Verbose::VERBOSITY_NORMAL);
    mpLocalMapper->RequestStop();
    // Wait until Local Mapping has effectively stopped
    while(!mpLocalMapper->isStopped())
    {
    ...
    Verbose::PrintMess("MERGE-VISUAL: Local Map stopped", Verbose::VERBOSITY_NORMAL);
    ...
    for(size_t i=0; i<4; ++i){
        if(pMergeMap->IsInUse(static_cast<uint8_t>(i))){
            spCC_merge = pMergeMap->GetCCPtrs();
            for(std::set<CentralControl*>::const_iterator sit = spCC_merge.begin();sit!=spCC_merge.end();++sit)
            {
                CentralControl* pCC = *sit;
                std::cout<<"LOCKCOMM pcc_merge client id:"<<unsigned(pCC->mnClientId)<<std::endl;
                while(!pCC->LockComm()){usleep(1000);}
              //  while(!pCC->LockMapping()){usleep(1000);}

            }
            //bMergeCCs = true;
            break; //once it is used by another , it should stop all comms...
        }
    }
    spCC_current = pCurrentMap->GetCCPtrs();
    for(std::set<CentralControl*>::const_iterator sit = spCC_current.begin();sit!=spCC_current.end();++sit)
    {
        CentralControl* pCC = *sit;
        std::cout<<"LOCKCOMM pcc_current client id:"<<unsigned(pCC->mnClientId)<<std::endl;
        while(!pCC->LockComm()){usleep(1000);}
      //  while(!pCC->LockMapping()){usleep(1000);}
        pMergeMap->AddCCPtr(pCC);
        pCurrentMap->EraseCCPtr(pCC);
    }
    ...
    g2o::Sim3 g2oCorrectedScw = mg2oMergeScw; //TODO Check the transformation !!!!TONGJIANG:HOW TO CHECK TRANSFORMATION???
...
#endif
    {
        ...
        for(KeyFrame* pKFi : spLocalWindowKFs)
        {
            ...
            pKFi->SetPose(pKFi->mTcwMerge, true);
        ...
        for(MapPoint* pMPi : spLocalWindowMPs)
        {
            ...
            pMPi->SetWorldPos(pMPi->mPosMerge, true);
    ...
    Verbose::PrintMess("MERGE-VISUAL: Finish to update connections in the welding area", Verbose::VERBOSITY_NORMAL);

    bool bStop = false;
    Verbose::PrintMess("MERGE-VISUAL: Start local BA ", Verbose::VERBOSITY_NORMAL);
    ...
    if (mSensor==IMU_MONOCULAR || mSensor==IMU_STEREO)
    {
        Verbose::PrintMess("MERGE-VISUAL: Visual-Inertial", Verbose::VERBOSITY_NORMAL);
    ...
    //return;
    Verbose::PrintMess("MERGE-VISUAL: Finish the LBA", Verbose::VERBOSITY_NORMAL);
    ...
    else
    {
        ...
        Verbose::PrintMess("MERGE-VISUAL: Apply transformation to all elements of the old map", Verbose::VERBOSITY_NORMAL);
    ...
    if(bRelaunchBA && (!pCurrentMap->isImuInitialized() || (pCurrentMap->KeyFramesInMap()<200 && mpAtlas->CountMaps()<4)))//why here only one map in atlas?:just to limit the number?
    {
        // Launch a new thread to perform Global Bundle Adjustment
        Verbose::PrintMess("Relaunch Global BA", Verbose::VERBOSITY_NORMAL);
        mbRunningGBA = true;
        mbFinishedGBA = false;
        mbStopGBA = false;
    ...
    pMergeMap->IncreaseChangeIndex();
    //std::cout<<"BEFORE RemoveBadMaps, mpAtlas size:"<<mpAtlas->CountMaps()<<std::endl;
    mpAtlas->RemoveBadMaps();
    std::set<CentralControl*> spCC = pMergeMap->GetCCPtrs(); //get all fused
    for(std::set<CentralControl*>::const_iterator sit = spCC.begin();sit!=spCC.end();++sit)
    {
        CentralControl* pCC = *sit;
        std::cout<<"To UnLockCOMM pCC client id:"<<unsigned(pCC->mnClientId)<<std::endl;
        pCC->UnLockComm();
      /*  if(!bRelaunchBA)
          pCC->UnLockMapping(); //not sure if this is wise*/
        if(spCC_current.count(pCC) && mSensor==MONOCULAR){
            pCC->mg2oS_wcurmap_wclientmap = mSold_new * pCC->mg2oS_wcurmap_wclientmap;
            std::cout<<GREEN<<"Client"<<unsigned(pCC->mnClientId)<<"changed its scale to:"<<pCC->mg2oS_wcurmap_wclientmap.scale()<<RST<<std::endl;
        }
...
void LoopClosing::MergeLocal2()
{
    ...
    Map* pCurrentMap = mpCurrentKF->GetMap();
    Map* pMergeMap = mpMergeMatchedKF->GetMap();
    std::cout<<"matched kf:"<<mpMergeMatchedKF->mnId<<std::endl;
...

    {
        ...
        std::cout<<"s_on"<<s_on<<std::endl;
        std::cout<<"R_on"<<R_on<<std::endl;
        std::cout<<"t_on"<<t_on<<std::endl;
        std::cout<<"c time:"<<setprecision(15)<<mpCurrentKF->mTimeStamp<<", c pose in c map:"<<setprecision(5)<<mpCurrentKF->GetPose()<<",client:"<<unsigned(mpCurrentKF->mnClientId)<<std::endl;
        std::cout<<"m time:"<<setprecision(15)<<mpMergeMatchedKF->mTimeStamp<<", m pose in m map:"<<setprecision(5)<<mpMergeMatchedKF->GetPose()<<unsigned(mpMergeMatchedKF->mnClientId)<<std::endl;

        unique_lock<mutex> lock(mpAtlas->GetCurrentMap()->mMutexMapUpdate);
        //unique_lock<mutex> currentLock(pCurrentMap->mMutexMapUpdate); // We remove the Kfs and MPs in the merged area from the old map


        cout << "KFs before empty: " << mpAtlas->GetCurrentMap()->KeyFramesInMap() << endl;
        mpLocalMapper->EmptyQueue(); //TONGJIANG: NOW IT ALMOST DOES NOTHING
      ...
      //  cout << "curr tracking KF id: " << mpTracker->GetLastKeyFrame()->mnId << endl;
        ...
        if(s_on!=1)
            bScaleVel=true;
        mpAtlas->GetCurrentMap()->ApplyScaledRotation(R_on,s_on,bScaleVel,false,t_on);
    ...
    //if((mSensor==IMU_MONOCULAR || mSensor==IMU_STEREO)&& !pCurrentMap->GetInertialBA2()){
    if((mSensor==IMU_MONOCULAR || mSensor==IMU_STEREO)&& !mpCurrentKF->GetInertialBA2()){
    ...
    cout << "MergeMap init ID: " << pMergeMap->GetInitKFid() << "of Client:" << unsigned(pMergeMap->mnClientId) <<
     "       CurrMap init ID: " << pCurrentMap->GetInitKFid() << "of Client:" << unsigned(pCurrentMap->mnClientId) << endl;
    ...
    cout << "updating current map" << endl;
    {
        ...
        vector<KeyFrame*> vpKFs = pMergeMap->GetAllKeyFrames();
    ...
    //Tongjiang: here pMergeMap should be empty
    cout << "end updating current map" << endl;

    // Critical zone
    //pMergeMap->PrintEssentialGraph();
    cout << "1" << endl;
    bool good = pMergeMap->CheckEssentialGraph();
    if(!good)
    ...
    cout << "MergeMap size: " << pMergeMap->GetAllKeyFrames().size() << "       CurrMap size : " << pCurrentMap->GetAllKeyFrames().size() << endl;
    cout << "MergeMap init ID: " << pMergeMap->GetInitKFid() << "       CurrMap init ID: " << pCurrentMap->GetInitKFid() << endl;
    ...
    //pMergeMap->PrintEssentialGraph();
    cout << "2" << endl;
    good = pMergeMap->CheckEssentialGraph();
    if(!good)
    ...
    if (mvpMergeConnectedKFs.size()>10)
        mvpMergeConnectedKFs.erase(mvpMergeConnectedKFs.begin()+10,mvpMergeConnectedKFs.end());
    /*mvpMergeConnectedKFs = mpMergeMatchedKF->GetVectorCovisibleKeyFrames();
    mvpMergeConnectedKFs.push_back(mpMergeMatchedKF);*/

    mpCurrentKF->UpdateConnections();//???should it be merge?
    cout<<"after connection, current kf's parent is:"<<mpCurrentKF->GetParent()->mnId<<endl;
    ...
    if (vpCurrentConnectedKFs.size()>6)
        vpCurrentConnectedKFs.erase(vpCurrentConnectedKFs.begin()+6,vpCurrentConnectedKFs.end()); //Tongjiang: why not just use GetBestCovisibilityKeyFrames??????
    ...
    for(size_t i=0; i<vpCurrentConnectedKFs.size();++i){
        cout<<"curr"<<vpCurrentConnectedKFs[i]->mnId<<","<<unsigned(vpCurrentConnectedKFs[i]->mnClientId)<<endl;
    }
    for(size_t i=0; i<mvpMergeConnectedKFs.size();++i){
        cout<<"merge"<<mvpMergeConnectedKFs[i]->mnId<<","<<unsigned(mvpMergeConnectedKFs[i]->mnClientId)<<endl;
    }
    ...
    vector<KeyFrame*> c = mpCurrentKF->GetVectorCovisibleKeyFrames();
    std::cout<<"connected size:"<<c.size()<<std::endl;
    for(KeyFrame* pKFi : c){
        if(mpCurrentKF->mnClientId != pKFi->mnClientId){
            std::cout<<"current kf"<<mpCurrentKF->mnId<<
            "is connected with kf from another client:"<<pKFi->mnId<<std::endl;
        }
    }
    vector<KeyFrame*> d = mpCurrentKF->GetBestCovisibilityKeyFrames(5);
    for(KeyFrame* pKFi : d){
        if(mpCurrentKF->mnClientId != pKFi->mnClientId){
            std::cout<<"best.current kf"<<mpCurrentKF->mnId<<
            "is connected with kf from another client:"<<pKFi->mnId<<std::endl;
        }
    }
    cout << "MergeMap init ID: " << pMergeMap->GetInitKFid() << "       CurrMap init ID: " << pCurrentMap->GetInitKFid() << endl;
    ...
    //PROBLEM: can pCurrKF not from Tracker?
  // KeyFrame* pCurrKF = mpTracker->GetLastKeyFrame();
    ...
    Optimizer::MergeInertialBA(mpCurrentKF, mpMergeMatchedKF, &bStopFlag, pMergeMap,CorrectedSim3);
    ...
    /*std::set<CentralControl*> spCC = pCurrentMap->GetCCPtrs(); //get all fused
    std::cout<<"spCC.size"<<spCC.size()<<std::endl;
    for(std::set<CentralControl*>::const_iterator sit = spCC.begin();sit!=spCC.end();++sit)
    {
        CentralControl* pCC = *sit;
        std::cout<<"To UnLockCOMM pCC client id:"<<unsigned(pCC->mnClientId)<<std::endl;
...
void LoopClosing::RunGlobalBundleAdjustment(Map* pActiveMap, unsigned long nLoopKF)
{
    ...
    if(!bImuInit){
        std::cout<<"before GBA"<<std::endl;
        Optimizer::GlobalBundleAdjustemnt(pActiveMap,10,&mbStopGBA,true, nLoopKF,false);
        std::cout<<"after GBA"<<std::endl;
    }
    else
        Optimizer::FullInertialBA(pActiveMap,7,false,nLoopKF,true, &mbStopGBA);
  ...
  /*  std::set<CentralControl*> spCC = pActiveMap->GetCCPtrs();
    for(std::set<CentralControl*>::const_iterator sit = spCC.begin();sit!=spCC.end();++sit)
    {
        CentralControl* pCC = *sit;
        std::cout<<"To UnLockMapping pCC client id:"<<unsigned(pCC->mnClientId)<<std::endl;
        pCC->UnLockMapping();
    }*/
}
