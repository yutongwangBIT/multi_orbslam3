...
ClientViewer::ClientViewer(ClientSystem* pSystem, MapDrawer* pMapDrawer, FrameDrawer *pFrameDrawer):
mNh(pMapDrawer->mpCC->mNhPrivate), mbFinishRequested(false),mpSystem(pSystem), 
mpFrameDrawer(pFrameDrawer),mpMapDrawer(pMapDrawer),
mbFinished(true), mbStopped(true), mbStopRequested(false)
{
    mNh.param("Client/Viewer_mViewpointX",mViewpointX,0.0f);
    mNh.param("Client/Viewer_mViewpointY",mViewpointY,-0.7f);
    mNh.param("Client/Viewer_mViewpointZ",mViewpointZ,-1.8f);
    mNh.param("Client/Viewer_mViewpointF",mViewpointF,500.0f);
}
void ClientViewer::RunClient()
{
    mbFinished = false;
    mbStopped = false;
...
void ClientViewer::RequestFinish() //can be called in system shutdown TODO
{
    unique_lock<mutex> lock(mMutexFinish);
    mbFinishRequested = true;
}

bool ClientViewer::CheckFinish()
{
    unique_lock<mutex> lock(mMutexFinish);
    return mbFinishRequested;
}

void ClientViewer::SetFinish()
{
    unique_lock<mutex> lock(mMutexFinish);
    mbFinished = true;
}

bool ClientViewer::isFinished()
{
    unique_lock<mutex> lock(mMutexFinish);
    return mbFinished;
}

void ClientViewer::RequestStop()
{
...
bool ClientViewer::isStopped()
{
    unique_lock<mutex> lock(mMutexStop);
    return mbStopped;
}

bool ClientViewer::Stop()
{
    unique_lock<mutex> lock(mMutexStop);
