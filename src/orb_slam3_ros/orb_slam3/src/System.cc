    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false)
{
  ...
  /*  cv::FileStorage fsSettings(strSettingsFile.c_str(), cv::FileStorage::READ);
   ...
   // mpFrameDrawer = new FrameDrawer(mpAtlas);
    ...
    mpTracker = new Tracking(this, mpVocabulary, mpAtlas, mpKeyFrameDatabase, mSensor, parameters, strSequence);
    ...
    //mpLocalMapper = new LocalMapping(mpAtlas, mSensor==MONOCULAR || mSensor==IMU_MONOCULAR, mSensor==IMU_MONOCULAR || mSensor==IMU_STEREO, strSequence);
...
void System::InsertKeyFrame(KeyFrame *pKF){
    orb_slam3_ros::KF kf_msg = pKF->GetKFMessageClient();
    {
        std::unique_lock<std::mutex> lock(mMutexKFMsgBuffer);
        if(pKF->mnId!=0)
            //std::cout<<"id:"<<pKF->mnId<<std::endl;
            KFMsgBuffer.push_back(kf_msg);
    }
}

orb_slam3_ros::KFs System::GetKeyFramesBuffer(){
    std::unique_lock<std::mutex> lock(mMutexKFMsgBuffer);
    orb_slam3_ros::KFs KF_buffer_msg;
    if(!KFMsgBuffer.empty()){
        //std::cout<<KFMsgBuffer.size()<<std::endl;
        for(int i=0; i<KFMsgBuffer.size(); ++i){
            KF_buffer_msg.KFs.push_back(KFMsgBuffer.front());
            KFMsgBuffer.pop_front();
        }
    }
    return KF_buffer_msg;
}
