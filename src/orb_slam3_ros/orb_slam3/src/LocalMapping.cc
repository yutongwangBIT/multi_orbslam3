...
LocalMapping::LocalMapping(CentralControl* pCC, Atlas *pAtlas, ServerViewer* pViewer, const float bMonocular, bool bInertial, const string &_strSeqName):
    mpCC(pCC), mnClientId(pCC->mnClientId), mpViewer(pViewer), mNhPrivate(pCC->mNhPrivate),
    mbMonocular(bMonocular), mbInertial(bInertial), mbResetRequested(false), mbResetRequestedActiveMap(false), mbFinishRequested(false),
    mbFinished(true), mpAtlas(pAtlas), bInitializing(false),
...
/*LocalMapping::LocalMapping(Atlas *pAtlas, const float bMonocular, bool bInertial, const string &_strSeqName):
    ...
    mbAbortBA(false), mbStopped(false), mbStopRequested(false), mbNotStop(false), mbAcceptKeyFrames(true),
    mbNewInit(false), mbNoLP(true), mIdxInit(0), mScale(1.0), mInitSect(0), mbNotBA1(true), mbNotBA2(true), mIdxIteration(0), infoInertial(Eigen::MatrixXd::Zero(9,9))
{*/
    mpSystem = mpCC->mpCS;
    mnMatchesInliers = 0;
    ...
    if(mpCC->mSysState==eSystemState::CLIENT){
        ...
        mNhPrivate.param(node_name+"/LM/LocalBASize",LocalBASize,20);
        mNhPrivate.param(node_name+"/LockSleepLM",LockSleepLM,100);
    }
    else{
        mCountKFs = 0;
        mbNoLP = false;
...
void LocalMapping::RunServer(){
    ...
    while(1){
        if(!stopRequested()){
            if(CheckNewKeyFrames()){
                while(!mpCC->LockMapping()){usleep(LockSleepLM);}
                //while(!mpAtlas->LockMapUpdate()){usleep(1000);}
                ProcessNewKeyFrameServer();
        ...
        {
          //  std::cout<<"LM:want to stop"<<std::endl;
            // Safe area to stop
          ...
          //  std::cout<<"LM:Finished Stopping"<<std::endl;
            if(CheckFinish())
                break;
        }
        else
            std::cout<<"LM: stop or not stop? That's a question"<<std::endl;
    ...
    std::cout<<"LM:Finished"<<std::endl;
    SetFinish();
}
void LocalMapping::ProcessNewKeyFrameServer(){ //TONGJIANG: TODO
    {
...
void LocalMapping::RunClient()
{
    ...
    while(1)
    {
        ...
        //std::cout<<"LocalMapping is running"<<std::endl;


        // Check if there are keyframes in the queue
        if(CheckNewKeyFrames() && !mbBadImu)
        {
            //std::cout<<"LM:there is KF in the queue, loop started"<<std::endl;

            std::chrono::steady_clock::time_point t0 = std::chrono::steady_clock::now();
            // CAHNGED FOR COMMUNICATION
            while(!mpCC->LockMapping()){
                //std::cout<<"LM: is locking Mapping"<<std::endl;
                usleep(LockSleepLM);
            }
            // BoW conversion and insertion in Map
            ProcessNewKeyFrame();
            std::chrono::steady_clock::time_point t1 = std::chrono::steady_clock::now();
            //std::cout<<"LM:ProcessNewKeyFrame"<<std::endl;

            // Check recent MapPoints
            //MapPointCulling();
            std::chrono::steady_clock::time_point t2 = std::chrono::steady_clock::now();
            //std::cout<<"LM:MapPointCulling"<<std::endl;

            // Triangulate new MapPoints
            CreateNewMapPoints();
            std::chrono::steady_clock::time_point t3 = std::chrono::steady_clock::now();
            //std::cout<<"LM:CreateNewMapPoints"<<std::endl;
            // Save here:
            // # Cov KFs
            // # tot Kfs
            // # recent added MPs
            ...
            if(!CheckNewKeyFrames() && !stopRequested())
            {
                ...
                {
                    //std::cout<<"LM:11"<<std::endl;
                    if(mbInertial && mpCurrentKeyFrame->GetMap()->isImuInitialized())
                    {
                        //std::cout<<"LM:111"<<std::endl;
                        ...
                        //std::cout<<"LM:LocalInertialBA"<<std::endl;
                        //TODO::use LocalBASize like LBA
                    }
                    else
                    {
                        //std::cout<<"LM:222"<<std::endl;
                        std::chrono::steady_clock::time_point begin = std::chrono::steady_clock::now();
                        Optimizer::LocalBundleAdjustment(mpCurrentKeyFrame,&mbAbortBA, mpCurrentKeyFrame->GetMap(),num_FixedKF_BA, LocalBASize);
                ...
                if(!mpCurrentKeyFrame->GetMap()->isImuInitialized() && mbInertial)
                {
                    if (mbMonocular){
                      ...
                      //  std::cout<<"LM:InitializeIMU"<<std::endl;
                    }
                    else
                ...
                {
                    //std::cout<<"LM:mbInertial"<<std::endl;
                    if(mpCurrentKeyFrame->GetMap()->isImuInitialized() && mpTracker->mState==Tracking::OK) // Enter here everytime local-mapping is called
                    {
                        //std::cout<<"LM:isImuInitialized & OK"<<std::endl;
                        if(!mpCurrentKeyFrame->GetMap()->GetInertialBA1()){
                            //std::cout<<"LM:want VIBA1"<<std::endl;
                            if (mTinit>3.0f)
                            {
                        ...
                        //else if (mbNotBA2){
                            ...
                            //std::cout<<"LM:want VIBA2"<<std::endl;
                            if (mTinit>6.0f){ // 15.0f
                                ...
                                mpCurrentKeyFrame->GetMap()->SetInertialBA2();//CHANGED ITS POSTITION
                        ...
                        /*if (((mpAtlas->KeyFramesInMap())<=100) &&
                                ((mTinit>15.0f && mTinit<15.9f)||
                                (mTinit>25.0f && mTinit<25.9f)||
                                (mTinit>45.0f && mTinit<45.9f)||
                                (mTinit>55.0f && mTinit<55.9f)||
                                (mTinit>65.0f && mTinit<65.9f)||
                                (mTinit>75.0f && mTinit<75.9f))){
            ...
            //--
            mpCC->UnLockMapping();
            //std::cout<<"LocalMapping loop ended"<<std::endl;
        }
...
void LocalMapping::ProcessNewKeyFrame()
{
    ...
    //std::cout<<"LM:i can add keyframe to atlas"<<std::endl;
...
void LocalMapping::MapPointCullingServer()
{
    ...
    while(lit!=mlpRecentAddedMapPoints.end())
    {
        ...
        else if((mCountKFs - pMP->mInsertedWithKF) >= 3 && pMP->mnClientId == mpCC->mnClientId && pMP->Observations() <= cnThObs)
        {
            pMP->SetBadFlag();
            //std::cout<<"LM SetBadFlag"<<std::endl;
            lit = mlpRecentAddedMapPoints.erase(lit);
        }
...
void LocalMapping::CreateNewMapPoints()
{
    ...
    for(size_t i=0; i<vpNeighKFs.size(); i++)
    {
        ...
        bool bCoarse = mbInertial &&
                ((!mpCurrentKeyFrame->GetMap()->GetInertialBA2() && mpCurrentKeyFrame->GetMap()->GetInertialBA1())||
        ...
        for(int ikp=0; ikp<nmatches; ikp++)
        {
            ...
            MapPoint* pMP = new MapPoint(x3D,mpCurrentKeyFrame,mpAtlas->GetCurrentMap(), mpComm, mpCC->mSysState, -1, mnClientId);
            ...
            /*if(pKF2->mnClientId!=mnClientId)
                std::cout<<"add new mp to kf from another"<<std::endl;*/
...
void LocalMapping::RequestStop()
{
  ...
  /*  if(mbStopRequested)
        std::cout<<"mbStopRequested"<<std::endl;*/
...
void LocalMapping::KeyFrameCulling()
{
    ...
    /*    */
        ...
        {
            //std::cout<<"nRedundantObservations:"<<nRedundantObservations<<std::endl;
            //std::cout<<"nMPs:"<<nMPs<<std::endl;
            //std::cout<<"redundant_th*nMPs:"<<redundant_th*nMPs<<std::endl;
            //std::cout<<"Satisfied kf:"<<pKF->mnId<<std::endl;
            if (mbInertial)
            {
                ...
                if(mpCC->mSysState==eSystemState::CLIENT)
                {
                    if(pKF->mPrevKF && pKF->mNextKF)
                    {
                        ...
                        else if(!mpCurrentKeyFrame->GetMap()->GetInertialBA2() && (cv::norm(pKF->GetImuPosition()-pKF->mPrevKF->GetImuPosition())<0.02) && (t<3))
                        {
                            pKF->mNextKF->mpImuPreintegrated->MergePrevious(pKF->mpImuPreintegrated);
                            pKF->mNextKF->mPrevKF = pKF->mPrevKF;
                            pKF->mPrevKF->mNextKF = pKF->mNextKF;
                    ...
                    if(pKF->mPrevKF && pKF->mNextKF)//check mNextKF in SERVER
                    {
                        const float ts = pKF->mNextKF->mTimeStamp-pKF->mPrevKF->mTimeStamp;
                        //std::cout<<"ts:"<<ts<<std::endl;
                        if((bInitImu && (pKF->mnId<last_ID) && ts<3.) || (ts<0.5))//check lastID?
                        {
                            std::cout<<"KF:"<<pKF->mnId<<"to be erased, its prev:"<<pKF->mPrevKF->mnId<<", its next:"<<pKF->mNextKF->mnId<<std::endl;
                            pKF->mmTcPrev = pKF->mmTcPrevOri * pKF->mPrevKFOri->mmTcPrev;
                            pKF->mNextKF->mpImuPreintegrated->MergePrevious(pKF->mpImuPreintegrated);
                            pKF->mNextKF->mPrevKF = pKF->mPrevKF;
                            pKF->mPrevKF->mNextKF = pKF->mNextKF;
                            pKF->mNextKF = NULL;
                    ...
                    else if(pKF->mPrevKF && !pKF->mNextKF){
                        std::cout<<"to be erased kf has no next kf"<<std::endl;
                    }
                    // we can also erase the kf without mNextKF,,,,here just for simplity.
                }
...
void LocalMapping::RequestFinish()
{
    ...
    std::cout<<"LM: RequestFinish is called"<<std::endl;
...
void LocalMapping::InitializeIMU(float priorG, float priorA, bool bFIBA)
{
    ...
    if (!mpCurrentKeyFrame->GetMap()->isImuInitialized())
    {
        ...
        for(vector<KeyFrame*>::iterator itKF = vpKF.begin(); itKF!=vpKF.end(); itKF++)
        {
          ...
          //  std::cout<<"prev IMU rotation:"<<(*itKF)->mPrevKF->GetImuRotation()<<std::endl;
          //  std::cout<<"mpImuPreintegrated GetUpdatedDeltaVelocity:"<<(*itKF)->mpImuPreintegrated->GetUpdatedDeltaVelocity()<<std::endl;
    ...
    /*cout << "bg before inertial-only optimization: " << mbg << endl;
    cout << "ba before inertial-only optimization: " << mba << endl;
    cout << "mRwg before inertial-only optimization: " << mRwg << endl;*/

    //INERTIAL ONLY OPTIMIZATION MEANS THAT POSE IS NOT UPDATED
    std::chrono::steady_clock::time_point t0 = std::chrono::steady_clock::now();
    Optimizer::InertialOptimization(mpAtlas->GetCurrentMap(), mRwg, mScale, mbg, mba, mbMonocular, infoInertial, false, false, priorG, priorA);
    std::chrono::steady_clock::time_point t1 = std::chrono::steady_clock::now();
    std::cout << "InertialOptimization time = " << std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0).count() << "[ms]" << std::endl;

    cout << "scale after inertial-only optimization: " << mScale << endl;
    //cout << "mRwg after inertial-only optimization: " << mRwg << endl;
    ...
    {
        mpAtlas->GetCurrentMap()->ApplyScaledRotation(Converter::toCvMat(mRwg).t(),mScale,true,false);
        mpTracker->UpdateFrameIMU(mScale,vpKF[0]->GetImuBias(),mpCurrentKeyFrame);
    }
    std::chrono::steady_clock::time_point t3 = std::chrono::steady_clock::now();
    std::cout << "mMutexMapUpdate time = " << std::chrono::duration_cast<std::chrono::milliseconds>(t3 - t2).count() << "[ms]" << std::endl;
    ...
    cout << "mpCurrentKeyFrame pose: " << mpCurrentKeyFrame->GetPose() << endl;

    std::chrono::steady_clock::time_point t4 = std::chrono::steady_clock::now();
    if (bFIBA)
    {
        if (priorA!=0.f)
            Optimizer::FullInertialBA(mpAtlas->GetCurrentMap(), 100, false, 0, false, NULL, true, priorG, priorA);
        else
            Optimizer::FullInertialBA(mpAtlas->GetCurrentMap(), 100, false, 0, false, NULL, false);
        //cout << "(after FullInertialBA) mpCurrentKeyFrame pose: " << mpCurrentKeyFrame->GetPose() << endl;
    }
    std::chrono::steady_clock::time_point t5 = std::chrono::steady_clock::now();
    std::cout << "FullInertialBA time = " << std::chrono::duration_cast<std::chrono::milliseconds>(t5 - t4).count() << "[ms]" << std::endl;
...
void LocalMapping::ScaleRefinement()
{
    ...
    const int N = vpKF.size();  //Above actually no use (Tong)
    ...
    std::cout<<"scale after scale refinement is:"<<mScale<<std::endl;
    std::cout<<"mRwg after scale refinement is:"<<mRwg<<std::endl;
    if (mScale<1e-1) // 1e-1
    {
        cout << "scale too small" << endl;
        bInitializing=false;
    ...
    if ((fabs(mScale-1.f)>0.00001)||!mbMonocular)
    {
        std::cout<<"We are changing the map:ApplyScaledRotation"<<std::endl;
        mpAtlas->GetCurrentMap()->ApplyScaledRotation(Converter::toCvMat(mRwg).t(),mScale,true,false); //The map is changing because mScale is not that close to 1
