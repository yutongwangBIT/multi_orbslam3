...
Tracking::Tracking(CentralControl* pCC, uint8_t ClientId, ORBVocabulary* pVoc, FrameDrawer* pFrameDrawer, Atlas *pAtlas, KeyFrameDatabase* pKFDB, const int sensor, ORBParameters& parameters, const string &_nameSeq):
    mpCC(pCC), mnClientId(ClientId), mState(NO_IMAGES_YET), mSensor(sensor), mTrackedFr(0), mbStep(false),
    mbOnlyTracking(false), mbMapUpdated(false), mbVO(false), mpORBVocabulary(pVoc), mpKeyFrameDB(pKFDB), mNh(pCC->mNh), mNhPrivate(pCC->mNhPrivate),
    mpInitializer(static_cast<Initializer*>(NULL)), mpAtlas(pAtlas), mnLastRelocFrameId(0), time_recently_lost(5.0),
    mnInitialFrameId(0), mbCreatedMap(false), mnFirstFrameId(0), mpCamera2(nullptr), mbNoLP(true), mpFrameDrawer(pFrameDrawer)
{
    mpSystem = mpCC->mpCS;
    ...
    string camera_type = parameters.camera_type;
    mbf = parameters.camera_bf;
    ...
    else if(camera_type == "KannalaBrandt8")
    {
        ...
        std::cout << "- mDistCoef: " << mDistCoef.at<float>(0) << std::endl;
    ...
    mpAtlas->AddCamera(mpCamera); //TODO: STEREO
    std::cout<<"Tracking ini, atlas added camera"<<std::endl;
    ...
    if(sensor==STEREO || mSensor==IMU_STEREO)
    ...
    if(sensor==RGBD)
    {
        ...
        cout << endl << "mDepthMapFactor: " << mDepthMapFactor << endl;
    }
    ...
    mNhPrivate.param(node_name+"/LockSleepTracking",LockSleepTracking,1000);
...
/*bool Tracking::ParseCamParamFile(cv::FileStorage &fSettings)
{
    ...
    if(mSensor==STEREO || mSensor==RGBD || mSensor==IMU_STEREO)
    {
...
void Tracking::SetCommunicator(Communicator* pComm){
    mpComm = pComm;
}
...
cv::Mat Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename)
{
    ...
    mImColor = mImGray;
    if (mSensor == STEREO && !mpCamera2)
        mCurrentFrame = Frame(mnClientId, mImGray,imGrayRight,timestamp,mpORBextractorLeft,mpORBextractorRight,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera);
    else if(mSensor == STEREO && mpCamera2)
        mCurrentFrame = Frame(mnClientId, mImGray,imGrayRight,timestamp,mpORBextractorLeft,mpORBextractorRight,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,mpCamera2,mTlr);
    else if(mSensor == IMU_STEREO && !mpCamera2)
        mCurrentFrame = Frame(mnClientId, mImGray,imGrayRight,timestamp,mpORBextractorLeft,mpORBextractorRight,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,&mLastFrame,*mpImuCalib);
    else if(mSensor == IMU_STEREO && mpCamera2)
        mCurrentFrame = Frame(mnClientId, mImGray,imGrayRight,timestamp,mpORBextractorLeft,mpORBextractorRight,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,mpCamera2,mTlr,&mLastFrame,*mpImuCalib);
...
cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp, string filename)
{
    ...
    std::chrono::steady_clock::time_point t0 = std::chrono::steady_clock::now();
    mCurrentFrame = Frame(mnClientId, mImGray,imDepth,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera);
    ...
    std::cout<<"Tracking:track time:"<<setprecision(6) << t_track<<std::endl;
...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, string filename)
{
    ...
    if (mSensor == MONOCULAR)
    {
        if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET ||(lastID - initID) < mMaxFrames){
            mCurrentFrame = Frame(mnClientId, mImGray,timestamp,mpIniORBextractor,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth);}
        else
            mCurrentFrame = Frame(mnClientId, mImGray,timestamp,mpORBextractorLeft,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth);
    }
    else if(mSensor == IMU_MONOCULAR)
    {
        if(mpImuCalib->IsEmpty())
            std::cout<<RED<<"ERROR: NO IMU CALIBRATION PARAMETERS!"<<RST<<std::endl;
        if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET)
        {
            std::cout<<"NOT_INITIALIZED"<<std::endl;
            mCurrentFrame = Frame(mnClientId, mImGray,timestamp,mpIniORBextractor,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth,&mLastFrame,*mpImuCalib);
        }
        else{
            std::cout<<"INITIALIZED"<<std::endl;
            mCurrentFrame = Frame(mnClientId, mImGray,timestamp,mpORBextractorLeft,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth,&mLastFrame,*mpImuCalib);
        }
        std::cout<<"FRAME contructed"<<std::endl;
    }
...
void Tracking::PreintegrateIMU()
{
  ...
  //   cout << "start loop. Total meas:" << mlQueueImuData.size() << endl;
    ...
    //FOR DEBUG
/*    cv::Mat cm1 = mpImuPreintegratedFromLastKF->GetDeltaPosition(mCurrentFrame.mImuBias);
    cv::Mat cm2 = mpImuPreintegratedFromLastKF->GetDeltaVelocity(mCurrentFrame.mImuBias);
    std::cout<<"Preintegration delta position:"<<cm1<<std::endl;
    std::cout<<"Preintegration delta velocity:"<<cm2<<std::endl;
    std::cout<<"Preintegration delta rotation:"<<mpImuPreintegratedFromLastKF->GetDeltaRotation(mCurrentFrame.mImuBias)<<std::endl;
    std::cout<<"Preintegration Jrg:"<<mpImuPreintegratedFromLastKF->JRg<<std::endl;
    std::cout<<"Preintegration JVg:"<<mpImuPreintegratedFromLastKF->JVg<<std::endl;
    std::cout<<"Preintegration JPg:"<<mpImuPreintegratedFromLastKF->JPg<<std::endl;
    std::cout<<"Preintegration JVa:"<<mpImuPreintegratedFromLastKF->JVa<<std::endl;
    std::cout<<"Preintegration JPa:"<<mpImuPreintegratedFromLastKF->JPa<<std::endl;
    std::cout<<"Preintegration dT:"<<mpImuPreintegratedFromLastKF->dT<<std::endl;
    std::cout<<"Preintegration C:"<<(mpImuPreintegratedFromLastKF->C).size()<<std::endl;
    std::cout<<"Preintegration info:"<<(mpImuPreintegratedFromLastKF->Info).size()<<std::endl;
    std::cout<<"Preintegration Nga:"<<(mpImuPreintegratedFromLastKF->Nga).size()<<std::endl;
    std::cout<<"Preintegration Nga walk:"<<(mpImuPreintegratedFromLastKF->NgaWalk).size()<<std::endl;
    std::cout<<"Preintegration avgA:"<<(mpImuPreintegratedFromLastKF->avgA).size()<<std::endl;
    std::cout<<"Preintegration avgW:"<<(mpImuPreintegratedFromLastKF->avgW).size()<<std::endl;*/
...
bool Tracking::PredictStateIMU()
{
    ...
    if(mbMapUpdated && mpLastKeyFrame)
    {
        ...
        //std::cout<<"PredictStateIMU mPredtwb:"<<mCurrentFrame.mPredtwb<<std::endl;
        //std::cout<<"PredictStateIMU mPredVwb:"<<mCurrentFrame.mPredtwb<<std::endl;
        return true;
    }
...
void Tracking::Track()
{
        ...
        else if(mCurrentFrame.mTimeStamp>mLastFrame.mTimeStamp+1.0)
        {
            ...
            if(mpAtlas->isInertial())
            {
                if(mpAtlas->isImuInitialized())
                {
                    ...
                    if(!pCurrentMap->GetInertialBA2()) //TONGJIANG:may be 1 or initialazed?
                    {
                        mpSystem->ResetActiveMap();
                    }
                    else
    ...
    if ((mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO) && mpLastKeyFrame)
    ...
    if ((mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO) && !mbCreatedMap)
    {
#ifdef SAVE_TIMES
        std::chrono::steady_clock::time_point t0 = std::chrono::steady_clock::now();
#endif
    ...
    {
        //cout << "Map update detected" << nCurMapChangeIndex << endl;
    ...
    else
    {
        ...
        while(!mpCC->LockTracking()){
            //std::cout<<"Tracking: is locked"<<std::endl;
            usleep(LockSleepTracking);
        }
        ...
        std::chrono::steady_clock::time_point timeStartPosePredict = std::chrono::steady_clock::now();
            ...
            if(mState==OK)
            {
                ...
                else
                {
                    ...
                    if(!bOK){
                        Verbose::PrintMess("TRACK: Track with motion model failed, Track reference Keyframe", Verbose::VERBOSITY_DEBUG);
                        bOK = TrackReferenceKeyFrame();
                    }
        ...
        else
        {
            ...
            else
            {
                if(!mbVO)
                {
                    Verbose::PrintMess("!mbVO", Verbose::VERBOSITY_NORMAL);
                    // In last frame we tracked enough MapPoints in the map
                    if(!mVelocity.empty())
                    {
                        Verbose::PrintMess("!mVelocity.empty TrackWithMotionModel", Verbose::VERBOSITY_NORMAL);
                        bOK = TrackWithMotionModel();
                    }
                    else
                    {
                        Verbose::PrintMess("else TrackReferenceKeyFrame", Verbose::VERBOSITY_NORMAL);
                        bOK = TrackReferenceKeyFrame();
                    }
                }
                else
        ...
        else if (mState == OK)
        {
            if (mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO)
            {
                ...
                if(!pCurrentMap->isImuInitialized() || !pCurrentMap->GetInertialBA1())
                {
                    cout << "IMU is not or recently initialized. Reseting active map..." << endl;
                    mpSystem->ResetActiveMap();
                }
        ...
        if((mCurrentFrame.mnId<(mnLastRelocFrameId+mnFramesToResetIMU)) && (mCurrentFrame.mnId > mnFramesToResetIMU) && ((mSensor == IMU_MONOCULAR) || (mSensor == IMU_STEREO)) && pCurrentMap->isImuInitialized())
        {
        ...
        if(bOK || mState==RECENTLY_LOST)
        {
            ...
            if(bNeedKF && (bOK|| (mState==RECENTLY_LOST && (mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO))))
                CreateNewKeyFrame();
        ...
        if(mState==LOST)
        {
            if(pCurrentMap->KeyFramesInMap()<=1)//TONGJIANG:previous is 5
            {
                mpCC->UnLockTracking();
                Verbose::PrintMess("Track lost and has less 1 KF in current map, resetting", Verbose::VERBOSITY_NORMAL);
                mpSystem->ResetActiveMap();
                return;
            }
            if ((mSensor == IMU_MONOCULAR) || (mSensor == IMU_STEREO))
                ...
                {
                    mpCC->UnLockTracking();
                    Verbose::PrintMess("Track lost before IMU initialisation, reseting...", Verbose::VERBOSITY_NORMAL);
                    mpSystem->ResetActiveMap();
                    return;
                }
        ...
        //CHANGED FOR COMMUNICATION
        mpCC->UnLockTracking();
        //std::cout<<"Tracking: Unlock"<<std::endl;
    }
...
void Tracking::StereoInitialization()
{
    if(mCurrentFrame.N>500)
    {
        ...
        // Create KeyFrame
        KeyFrame* pKFini = new KeyFrame(mCurrentFrame,mpAtlas->GetCurrentMap(),mpKeyFrameDB, mpComm, eSystemState::CLIENT, -1);
        ...
        if(!mpCamera2){
            for(int i=0; i<mCurrentFrame.N;i++)
            {
                ...
                if(z>0)
                {
                    ...
                    MapPoint* pNewMP = new MapPoint(x3D,pKFini,mpAtlas->GetCurrentMap(), mpComm, eSystemState::CLIENT, -1, mnClientId);
...
void Tracking::MonocularInitialization()
{
    ...
    {
        if (((int)mCurrentFrame.mvKeys.size()<=100)||((mSensor == IMU_MONOCULAR)&&(mLastFrame.mTimeStamp-mInitialFrame.mTimeStamp>1.0)))
        {
            delete mpInitializer;
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    // Create KeyFrames
    std::cout<<"I am creating the initial map"<<std::endl;
    KeyFrame* pKFini = new KeyFrame(mInitialFrame,mpAtlas->GetCurrentMap(),mpKeyFrameDB, mpComm, eSystemState::CLIENT, -1);
    KeyFrame* pKFcur = new KeyFrame(mCurrentFrame,mpAtlas->GetCurrentMap(),mpKeyFrameDB, mpComm, eSystemState::CLIENT, -1);
    ...
    for(size_t i=0; i<mvIniMatches.size();i++)
    {
        ...
        MapPoint* pMP = new MapPoint(worldPos,pKFcur,mpAtlas->GetCurrentMap(), mpComm, eSystemState::CLIENT, -1, mnClientId);
    ...
    // Bundle Adjustment
    Verbose::PrintMess("Tracking: New Map created with " + to_string(mpAtlas->MapPointsInMap()) + " points", Verbose::VERBOSITY_QUIET);
    ...
    if (mSensor == IMU_MONOCULAR)
    {
        ...
        //std::cout<<"pKFcur->mPrevKF id:"<<pKFcur->mPrevKF->mnId<<std::endl;
        //std::cout<<"pKFini->mNextKF id:"<<pKFini->mNextKF->mnId<<std::endl;
...
void Tracking::CreateMapInAtlas()
{
    ...
    if((mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO ) && mpImuPreintegratedFromLastKF)
    {
        delete mpImuPreintegratedFromLastKF;
        mpImuPreintegratedFromLastKF = new IMU::Preintegrated(IMU::Bias(),*mpImuCalib);
    }
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    //cout << " TrackReferenceKeyFrame mLastFrame.mTcw:  "<< endl;
    // Discard outliers
    int nmatchesMap = 0;
    for(int i =0; i<mCurrentFrame.N; i++)
    {
...
void Tracking::UpdateLastFrame()
{
    ...
    if(mnLastKeyFrameId==mLastFrame.mnId || mSensor==MONOCULAR || mSensor==IMU_MONOCULAR || !mbOnlyTracking)
        return;
    ...
    for(size_t j=0; j<vDepthIdx.size();j++)
    {
        ...
        if(bCreateNew)
        {
            ...
            MapPoint* pNewMP = new MapPoint(x3D,mpAtlas->GetCurrentMap(),&mLastFrame,i, mpComm, eSystemState::CLIENT, -1, mnClientId);
...
bool Tracking::TrackWithMotionModel()
{
    ...
    if (mpAtlas->isImuInitialized() && (mCurrentFrame.mnId>mnLastRelocFrameId+mnFramesToResetIMU))
    {
        ...
        PredictStateIMU();
        //std::cout<<"PredictStateIMU"<<std::endl;
        return true;
    }
    else
    {
    ...
    int nmatches = matcher.SearchByProjection(mCurrentFrame,mLastFrame,th,mSensor==MONOCULAR || mSensor==IMU_MONOCULAR);
    ...
    if(nmatches<20)
    {
        ...
        nmatches = matcher.SearchByProjection(mCurrentFrame,mLastFrame,2*th,mSensor==MONOCULAR || mSensor==IMU_MONOCULAR);
    ...
    //std::cout<<"PoseOptimization"<<std::endl;
    Optimizer::PoseOptimization(&mCurrentFrame);
    //std::cout<<"PoseOptimization f"<<std::endl;
    // Discard outliers
    int nmatchesMap = 0;
    for(int i =0; i<mCurrentFrame.N; i++)
    {
...
bool Tracking::TrackLocalMap()
{
    ...
    int inliers;
    if (!mpAtlas->isImuInitialized()){
        //Verbose::PrintMess("TLM: IMU not initialized and PoseOptimization ", Verbose::VERBOSITY_NORMAL);
        Optimizer::PoseOptimization(&mCurrentFrame);
        //Verbose::PrintMess("TLM: FFF ", Verbose::VERBOSITY_NORMAL);
    ...
    else
    {
        ...
        else
        {
            ...
            if(!mbMapUpdated) //  && (mnMatchesInliers>30))
            {
                ...
                inliers = Optimizer::PoseInertialOptimizationLastFrame(&mCurrentFrame); // , !mpLastKeyFrame->GetMap()->GetInertialBA1());
                //Verbose::PrintMess("TLM: PoseInertialOptimizationLastFrame FFF", Verbose::VERBOSITY_NORMAL);

            }
            else
            {
                //Verbose::PrintMess("TLM: PoseInertialOptimizationLastKeyFrame ", Verbose::VERBOSITY_NORMAL);
                inliers = Optimizer::PoseInertialOptimizationLastKeyFrame(&mCurrentFrame); // , !mpLastKeyFrame->GetMap()->GetInertialBA1());
                //Verbose::PrintMess("TLM: PoseInertialOptimizationLastKeyFrame FFF ", Verbose::VERBOSITY_NORMAL);
    ...
    if (mSensor == IMU_MONOCULAR)
    {
        ...
        {
            std::cout<<"IMU_MONOCULAR mnMatchesInliers:"<<mnMatchesInliers<<std::endl;
            return false;
        }
        else
            return true;
...
{
    if(((mSensor == IMU_MONOCULAR) || (mSensor == IMU_STEREO)) && !mpAtlas->GetCurrentMap()->isImuInitialized())
    {
        if (mSensor == IMU_MONOCULAR && (mCurrentFrame.mTimeStamp-mpLastKeyFrame->mTimeStamp)>=0.25)
            return true;
        else if (mSensor == IMU_STEREO && (mCurrentFrame.mTimeStamp-mpLastKeyFrame->mTimeStamp)>=0.25)
            return true;
        else
            return false;
    }
    ...
    const bool c1c = mSensor!=MONOCULAR && mSensor!=IMU_MONOCULAR && mSensor!=IMU_STEREO && (mnMatchesInliers<nRefMatches*0.25 || bNeedToInsertClose) ;
    ...
    bool c4 = false;
    if ((((mnMatchesInliers<75) && (mnMatchesInliers>15)) || mState==RECENTLY_LOST) && ((mSensor == IMU_MONOCULAR))) // MODIFICATION_2, originally ((((mnMatchesInliers<75) && (mnMatchesInliers>15)) || mState==RECENTLY_LOST) && ((mSensor == IMU_MONOCULAR)))
        c4=true;
    else
        c4=false;
...
void Tracking::CreateNewKeyFrame()
{
    ...
    KeyFrame* pKF = new KeyFrame(mCurrentFrame,mpAtlas->GetCurrentMap(),mpKeyFrameDB, mpComm, eSystemState::CLIENT, -1);
    ...
    if(mpLastKeyFrame)
    {
        ...
        //std::cout<<"pKF->mPrevKF id:"<<pKF->mPrevKF->mnId<<std::endl;
    ...
    if(mSensor!=MONOCULAR && mSensor != IMU_MONOCULAR) // TODO check if incluide imu_stereo
    {
        ...
        if(!vDepthIdx.empty())
        {
            ...
            for(size_t j=0; j<vDepthIdx.size();j++)
            {
                ...
                if(bCreateNew)
                {
                    ...
                    MapPoint* pNewMP = new MapPoint(x3D,pKF,mpAtlas->GetCurrentMap(), mpComm, eSystemState::CLIENT, -1, mnClientId);
...
void Tracking::SearchLocalPoints()
{
    ...
    if(nToMatch>0)
    {
        ...
        else if(!mpAtlas->isImuInitialized() && (mSensor==IMU_MONOCULAR || mSensor==IMU_STEREO))
        {
            th=10;
        }
...
void Tracking::UpdateLocalPoints()
{
    ...
    {
        KeyFrame* pKF = *itKF;
        /*if(pKF->mnClientId!=mpLastKeyFrame->mnClientId)
            std::cout<<"Tracking: new kf in mvpLocalKeyFrames"<<std::endl;*/
...
void Tracking::UpdateLocalKeyFrames()
{
    ...
    if((mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO) &&mvpLocalKeyFrames.size()<80)
    {
...
void Tracking::Reset(bool bLocMap)
{
    ...
    Verbose::PrintMess("Reset End reseting! ", Verbose::VERBOSITY_NORMAL);
}
...
void Tracking::ResetActiveMap(bool bLocMap)
{
   ...
   // if(mpViewer)
    ...
    Verbose::PrintMess("ResetActiveMap End reseting! ", Verbose::VERBOSITY_NORMAL);
}
...
void Tracking::CreateNewMapPoints()
{
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        ...
        for(int ikp=0; ikp<nmatches; ikp++)
        {
            ...
            MapPoint* pMP = new MapPoint(x3D,mpLastKeyFrame,mpAtlas->GetCurrentMap(), mpComm, eSystemState::CLIENT, -1, mnClientId);
