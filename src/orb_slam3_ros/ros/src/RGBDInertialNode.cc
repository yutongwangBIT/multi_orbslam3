...
int main(int argc, char **argv) {
  ...
  ImuGrabber imugb;
  ImageGrabber igb(&client_system_,&imugb,true);
 ...
 /* message_filters::Subscriber<sensor_msgs::Image> rgb_subscriber_(NhPrivate, "/camera/rgb/image_raw", 1);
  ...
  std::thread sync_thread(&ImageGrabber::rgbdSyncWithImu,&igb);
...
void ImageGrabber::rgbdSyncWithImu(){
  while(1)
  {
    cv::Mat imRGB;
    cv::Mat imD;
    ...
    if (!rgbdBuf.empty()&&!mpImuGb->imuBuf.empty())
    {
      ...
      mpSLAM->TrackMonocular(imRGB,tIm,vImuMeas);
    }
...
ImageGrabber::ImageGrabber(ORB_SLAM3::ClientSystem* pSLAM, ImuGrabber *pImuGb, const bool bClahe):
